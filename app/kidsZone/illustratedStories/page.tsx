...
export const generateMetadata = createGenerateMetadata('illustratedStories_kidsZone');
...
export default async function Page({ searchParams }: any) {
  ...
  return (
    <>
      <PageLayout
        metaKey="illustratedStories_kidsZone"
        title={S('illustratedStories_kidsZone.title')}
        breadcrumbs={[{ labelKey: 'nav.home', href: '/' }, { label: String(t('illustratedStories.title')) }]}
        className=""
      >
        <p>{S('illustratedStories_kidsZone.description')}</p>
        <div className="card grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 md:gap-6">
          {(t('illustratedStories_kidsZone.kids_indian_stories', locale) as any[]).map((s: any) => (
