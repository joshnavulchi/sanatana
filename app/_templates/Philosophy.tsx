...
const PhilosophyTemplate: React.FC<PhilosophyTemplateProps> = ({ data }) => {
  const { t } = useLocale();
    ...
    <main className="min-h-screen bg-gradient-radial from-pink-100 via-white to-pink-300 dark:from-pink-900 dark:via-gray-900 dark:to-pink-800 flex items-center justify-center transition-colors duration-300">
      <section className="w-full max-w-6xl mx-auto py-16 px-6">
        <div className="relative mb-12">
          ...
          {data.description && (
            <p className="relative z-10 text-md md:text-lg md:text-xl text-pink-700 dark:text-pink-200 mb-8 font-medium tracking-wide">{data.description}</p>
          )}
        </div>
        <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-3">
          {data.philosophies.map((philosophy, idx) => (
            <article key={idx} className="group rounded-3xl shadow-2xl bg-gradient-to-br from-white via-pink-50 to-yellow-100 dark:from-gray-900 dark:via-pink-950 dark:to-yellow-900 border-2 border-pink-200 dark:border-pink-700 p-8 flex flex-col items-center hover:scale-105 hover:shadow-pink-400 dark:hover:shadow-pink-900 transition-transform duration-300 relative overflow-hidden">
              ...
              {philosophy.image && (
                <img src={philosophy.image} alt={philosophy.name} className="w-24 h-24 object-cover rounded-full mb-6 border-4 border-pink-200 dark:border-pink-700 shadow-lg" />
              )}
              <h2 className="text-2xl font-extrabold text-pink-800 dark:text-pink-200 mb-3 text-center drop-shadow-md">{philosophy.name}</h2>
              <p className="text-pink-700 dark:text-pink-100 text-base text-center leading-relaxed mb-2">{philosophy.summary}</p>
              {philosophy.link && (
                <a href={philosophy.link} className="text-pink-600 dark:text-pink-300 underline mt-2" target="_blank" rel="noopener noreferrer">{t('philosophy.read_more')}</a>
              )}
            </article>
          ))}
        </div>
