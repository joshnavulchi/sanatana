...
/* ── Veda icon map ── */
const vedas_page = {
  core_vedic: "Core Vedic Concepts",
  deities: "Deities",
  etymolog: "Etymology:",
  four_vedas: "The Four Vedas",
  hymns: "hymns",
  influenc_vedas: "Influence of the Vedas",
  introduc: "Introduction",
  k_10_mandalas: "The 10 Mandalas of Rigveda",
  key_themes: "Key Themes",
  major_deities: "Major Deities",
  major_rigvedic: "Major Rigvedic Deities",
  mandala: "Mandala",
  notable_hymns: "Notable Hymns",
  philosop_foundati: "Philosophical Foundations",
  related_concepts: "Related Concepts",
  rishis: "Rishis",
  theme: "Theme:",
  vedic_society: "Vedic Society",
  vedic_timeline: "Vedic Timeline"
};
...
const VEDA_ACCENTS: Record<string, {
  from: string;
  via: string;
  to: string;
  text: string;
...
function VedaCard({
  ...
  return <div className="relative overflow-hidden rounded-3xl border border-[#d8a25a]/40 bg-[#fffaf0] shadow-[0_10px_40px_rgba(122,46,31,0.10)] transition-all duration-300 hover:shadow-[0_20px_60px_rgba(166,61,23,0.16)]">
      {/* Top accent bar */}
      ...
      <div className="p-6 md:p-8 pl-5 md:pl-7">
        ...
        {importance && <div className="mt-4 flex items-center gap-2 text-sm font-semibold text-[#92400e]">
            <span className="text-base">✦</span> {importance}
          </div>}

        {/* Stats row */}
        {structure && <div className="mt-6 flex flex-wrap gap-3">
            {Object.entries(structure).map(([key, value]) => <div key={key} className="flex flex-col items-center rounded-xl border border-[#edc98f] bg-[#fffaf3] px-4 py-2.5 min-w-20">
        ...
        {divisions.length > 0 && <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3">
            {divisions.map((div: Record<string, unknown>, i: number) => <div key={i} className="rounded-xl border border-[#edc98f]/60 bg-[#fffaf3] p-4">
        ...
        {/* Deities chips */}
        {deities.length > 0 && <div className="mt-5">
            <h4 className="text-xs font-bold uppercase tracking-widest text-[#a89278] mb-2">{vedas_page.major_deities}</h4>
            <div className="flex flex-wrap gap-2">
              {deities.map((d: string) => <span key={d} className={`inline-flex items-center rounded-lg border ${accent.border}/40 bg-[#fffaf3] px-3 py-1 text-xs font-semibold ${accent.text}`}>
                  {d}
                </span>)}
            </div>
          </div>}
        ...
        {/* Theme/Topic tags */}
        {tags.length > 0 && <div className="mt-4">
            <h4 className="text-xs font-bold uppercase tracking-widest text-[#a89278] mb-2">{vedas_page.key_themes}</h4>
            <div className="flex flex-wrap gap-2">
              {tags.map((t: string, i: number) => <span key={i} className="inline-flex items-center gap-1 rounded-lg bg-[#fde7c7]/60 px-3 py-1 text-xs font-medium text-[#92400e]">
...
function MandalaRow({
  ...
  const shellBg = index % 3 === 0 ? 'bg-linear-to-br from-[#fffaf3] via-[#fef3e2] to-[#fbe8c8]' : index % 3 === 1 ? 'bg-linear-to-br from-[#fffbf5] via-[#fdf1dc] to-[#f8e4c0]' : 'bg-linear-to-br from-[#fff9f0] via-[#fce9ce] to-[#f5d9ae]';
  return <div className={`rounded-2xl border border-[#d8a25a]/30 overflow-hidden ${shellBg} transition-shadow duration-300 hover:shadow-[0_8px_30px_rgba(146,64,14,0.08)]`}>
      <button type="button" onClick={() => setOpen(!open)} className="w-full flex items-center gap-4 p-4 md:p-5 text-left cursor-pointer">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="text-base md:text-lg font-bold text-[#3d2e22]"> {vedas_page.mandala} {num} <span className="text-[#a89278] font-normal text-sm ml-2">— {hymns} {vedas_page.hymns}</span>
          </h4>
      ...
      {open && <div className="px-4 md:px-5 pb-5 pt-0 border-t border-[#edc98f]/40">
          {commentary && <p className="text-sm text-[#5b2d12] leading-relaxed mt-4">{commentary}</p>}

          {/* Rishis & Deities */}
          <div className="mt-4 flex flex-wrap gap-4">
            {allRishis.length > 0 && <div>
                <span className="text-[10px] font-bold uppercase tracking-widest text-[#a89278]">{vedas_page.rishis}</span>
                <div className="flex flex-wrap gap-1.5 mt-1">
                  {allRishis.map((r: string) => <span key={r} className="rounded-lg bg-[#fffaf3] border border-[#edc98f]/60 px-2.5 py-0.5 text-xs font-semibold text-[#92400e]">{r}</span>)}
                </div>
              </div>}
            {allDeities.length > 0 && <div>
                <span className="text-[10px] font-bold uppercase tracking-widest text-[#a89278]">{vedas_page.deities}</span>
                <div className="flex flex-wrap gap-1.5 mt-1">
                  {allDeities.map((d: string) => <span key={d} className="rounded-lg bg-[#fde7c7]/50 border border-[#d97706]/30 px-2.5 py-0.5 text-xs font-semibold text-[#7a2e1f]">{d}</span>)}
                </div>
              </div>}
          </div>

          {/* Themes */}
          {themes.length > 0 && <div className="mt-3 flex flex-wrap gap-2">
              {themes.map((t: string, i: number) => <span key={i} className="inline-flex items-center gap-1 rounded-lg bg-[#fde7c7]/40 px-2.5 py-1 text-[11px] font-medium text-[#5b2d12]">
                  <span className="h-1 w-1 rounded-full bg-[#d97706]" /> {t}
                </span>)}
            </div>}

          {/* Important hymns */}
          {allHymns.length > 0 && <div className="mt-5 space-y-3">
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#a89278]">{vedas_page.notable_hymns}</span>
              {allHymns.map((h: Record<string, unknown>, i: number) => <div key={i} className="rounded-xl border border-[#edc98f]/50 bg-[#fffaf3] p-3.5">
                  ...
                  {typeof h.theme === 'string' && <p className="text-[11px] font-medium text-[#92400e] mb-1">{vedas_page.theme} {h.theme}</p>}
...
function DeityCard({
  ...
  return <div className="relative overflow-hidden rounded-2xl border border-[#e0a632]/40 bg-[#fffaf0] p-5 shadow-[0_4px_20px_rgba(139,105,20,0.08)] hover:shadow-[0_12px_40px_rgba(139,105,20,0.14)] transition-all duration-300 hover:-translate-y-1">
...
export default function VedasClient() {
  ...
  if (isLoading && !ns?.title) {
    return <PageLayout metaKey="vedas" title="" breadcrumbs={[{
      label: 'Home',
      href: '/'
    }, {
      label: 'Vedas'
  ...
  return <PageLayout metaKey="vedas" title={title} breadcrumbs={[{
    label: 'Home',
    href: '/'
  }, {
    label: 'Vedas'
      ...
      <section className="relative overflow-hidden rounded-3xl border border-[#d8a25a]/30 bg-linear-to-br from-[#fffaf3] via-[#fdf0d7] to-[#fff8ef] p-6 md:p-10">
        ...
        <div className="relative z-10">
          ...
          {definition && <p className="text-lg md:text-xl text-[#5b2d12] leading-relaxed italic font-medium mb-6">{definition}</p>}

          {meaningOfWord && <div className="rounded-xl border border-[#e0a632]/40 bg-[#fffaf3] p-4 inline-block mb-4">
              <p className="text-sm text-[#92400e]">
                <span className="font-bold text-[#7a2e1f]">{vedas_page.etymolog}</span> {meaningOfWord}
              </p>
            </div>}

          {/* Stats badges */}
          {stats && <div className="flex flex-wrap gap-4 mt-6">
              {Object.entries(stats).map(([key, value]) => <div key={key} className="flex flex-col items-center rounded-xl border border-[#edc98f] bg-[#fffaf3] px-5 py-3 shadow-[0_4px_20px_rgba(122,46,31,0.08)]">
          ...
          {/* Composition period */}
          {period && <div className="flex flex-wrap gap-3 mt-5">
              {Object.entries(period).map(([key, value]) => <span key={key} className="inline-flex items-center gap-2 rounded-lg bg-[#fde7c7]/50 px-3 py-1 text-xs font-medium text-[#5b2d12]">
      ...
      {introduction && <>
          ...
          <section className="rounded-3xl border border-[#d8a25a]/20 bg-[#fffaf0] p-6 md:p-10 shadow-[0_8px_30px_rgba(146,64,14,0.06)]">
            <h2 className="text-2xl md:text-3xl font-extrabold bg-linear-to-r from-[#a63d17] via-[#d97706] to-[#f59e0b] bg-clip-text text-transparent mb-6"> {vedas_page.introduc} </h2>
      ...
      {scriptureText.length > 0 && <>
          <OrnamentDivider />
          <section>
            <h2 className="text-2xl md:text-3xl font-extrabold bg-linear-to-r from-[#92400e] via-[#c2410c] to-[#ea580c] bg-clip-text text-transparent mb-8 text-center"> {vedas_page.four_vedas} </h2>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {scriptureText.map((item: Record<string, unknown>, i: number) => <VedaCard key={i} item={item} />)}
            </div>
          </section>
        </>}
      ...
      {timeline && Object.keys(timeline).length > 0 && <>
          ...
          <section className="rounded-3xl border border-[#d8a25a]/20 bg-linear-to-br from-[#fffaf3] via-[#fef3e2] to-[#fbe8c8] p-6 md:p-10">
            <h2 className="text-2xl md:text-3xl font-extrabold bg-linear-to-r from-[#7c2d12] via-[#c2410c] to-[#fb923c] bg-clip-text text-transparent mb-8"> {vedas_page.vedic_timeline} </h2>
            <div className="relative ml-4">
              {/* Vertical line */}
              <div className="absolute left-4 top-0 bottom-0 w-0.5 bg-linear-to-b from-[#c2410c] to-[#f59e0b]" />
              <div className="space-y-6">
                {Object.entries(timeline).map(([key, value], i) => <div key={key} className="relative flex items-start gap-5 pl-6">
                    {/* Timeline dot */}
      ...
      {philosophicalExplanation && <>
          <OrnamentDivider />
          <section className="rounded-3xl border border-[#d8a25a]/20 bg-[#fffaf0] p-6 md:p-10 shadow-[0_8px_30px_rgba(146,64,14,0.06)]">
            <h2 className="text-2xl md:text-3xl font-extrabold bg-linear-to-r from-[#a63d17] via-[#d97706] to-[#f59e0b] bg-clip-text text-transparent mb-6"> {vedas_page.philosop_foundati} </h2>
            <div className="text-base text-[#5b2d12] leading-relaxed">
      ...
      {philoConcepts && Object.keys(philoConcepts).length > 0 && <>
          <OrnamentDivider />
          <section>
            <h2 className="text-2xl md:text-3xl font-extrabold bg-linear-to-r from-[#92400e] via-[#c2410c] to-[#ea580c] bg-clip-text text-transparent mb-8 text-center"> {vedas_page.core_vedic} </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {Object.entries(philoConcepts).map(([key, value]) => <div key={key} className="relative overflow-hidden rounded-2xl border border-[#d8a25a]/30 bg-[#fffaf0] p-5 hover:shadow-[0_8px_30px_rgba(146,64,14,0.10)] transition-all duration-200 hover:-translate-y-0.5">
                  <div className="absolute top-0 left-0 right-0 h-0.5 bg-linear-to-r from-[#8b6914] via-[#b8952e] to-[#e0a632]" />
      ...
      {deities.length > 0 && <>
          <OrnamentDivider />
          <section>
            <h2 className="text-2xl md:text-3xl font-extrabold bg-linear-to-r from-[#8b6914] via-[#b8952e] to-[#e0a632] bg-clip-text text-transparent mb-8 text-center"> {vedas_page.major_rigvedic} </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {deities.map((d: Record<string, unknown>, i: number) => <DeityCard key={i} deity={d} />)}
            </div>
          </section>
        </>}
      ...
      {mandalas.length > 0 && <>
          <OrnamentDivider />
          <section>
            <h2 className="text-2xl md:text-3xl font-extrabold bg-linear-to-r from-[#7c2d12] via-[#c2410c] to-[#fb923c] bg-clip-text text-transparent mb-8 text-center"> {vedas_page.k_10_mandalas} </h2>
            <div className="space-y-4">
              {mandalas.map((m: Record<string, unknown>, i: number) => <MandalaRow key={i} mandala={m} index={i} />)}
            </div>
          </section>
        </>}
      ...
      {society && Object.keys(society).length > 0 && <>
          ...
          <section className="rounded-3xl border border-[#d8a25a]/20 bg-[#fffaf0] p-6 md:p-10">
            <h2 className="text-2xl md:text-3xl font-extrabold bg-linear-to-r from-[#a63d17] via-[#d97706] to-[#f59e0b] bg-clip-text text-transparent mb-8"> {vedas_page.vedic_society} </h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {Object.entries(society).map(([key, items]) => {
            const list = Array.isArray(items) ? items : [];
            if (list.length === 0) return null;
            return <div key={key} className="rounded-2xl border border-[#edc98f]/50 bg-[#fffaf3] p-5">
                    <h4 className="text-sm font-bold uppercase tracking-widest text-[#92400e] mb-3">{key.replace(/_/g, ' ')}</h4>
                    <ul className="space-y-2">
                      {list.map((item: string, i: number) => <li key={i} className="flex items-start gap-2 text-sm text-[#5b2d12]">
      ...
      {influence && Object.keys(influence).length > 0 && <>
          <OrnamentDivider />
          <section>
            <h2 className="text-2xl md:text-3xl font-extrabold bg-linear-to-r from-[#92400e] via-[#c2410c] to-[#ea580c] bg-clip-text text-transparent mb-8 text-center"> {vedas_page.influenc_vedas} </h2>
            ...
            return <div key={key} className="relative overflow-hidden rounded-2xl border border-[#d8a25a]/30 bg-[#fffaf0] p-6">
      ...
      {relatedConcepts.length > 0 && <>
          ...
          <section className="rounded-3xl border border-[#d8a25a]/20 bg-linear-to-br from-[#fffaf3] via-[#fef3e2] to-[#fbe8c8] p-6 md:p-8 text-center">
            <h2 className="text-xl font-extrabold text-[#3d2e22] mb-5">{vedas_page.related_concepts}</h2>
            <div className="flex flex-wrap justify-center gap-3">
              {relatedConcepts.map((concept: string) => <span key={concept} className="inline-flex items-center rounded-xl border border-[#e0a632]/40 bg-[#fffaf3] px-4 py-2 text-sm font-semibold text-[#7a2e1f] shadow-[0_2px_10px_rgba(139,105,20,0.08)] hover:shadow-[0_4px_20px_rgba(139,105,20,0.14)] transition-shadow duration-200">
                  {concept}
                </span>)}
            </div>
          </section>
