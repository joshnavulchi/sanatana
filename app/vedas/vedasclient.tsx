  ...
  return (
    <PageLayout metaKey="vedas/index" title={title} description={description} breadcrumbs={[{ label: 'Home', href: '/' }, { label: title }]} className="layout-md">
      <div className="space-y-6">
        {data.definition && <p className="text-base">{data.definition}</p>}
        ...
        {data.meaning_of_word_veda && (
          <section className="mt-6">
            ...
            <p className="text-base">{data.meaning_of_word_veda}</p>
          </section>
        )}
          ...
          <section className="mt-6">
            ...
            <div className="flex flex-wrap gap-2">
              {data.related_concepts.map((r: any, i: number) => (
                <span key={i} className="text-sm px-2 py-1 bg-gray-100 rounded">{r}</span>
              ))}
            </div>
          </section>
        )}
