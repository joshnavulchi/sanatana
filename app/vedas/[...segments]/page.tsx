...
export default async function Page({ params }: Props) {
  ...
  const segments = ['vedas', ...rawSegments];
  ...
  // Breadcrumbs: Home > Vedas > seg1 > seg2 ...
  const crumbs = [{ label: 'Home', href: '/' }, { label: 'Vedas', href: '/vedas' }];
  let accum: string[] = [];
  for (const s of rawSegments) {
    accum.push(s);
    crumbs.push({ label: s, href: `/vedas/${accum.join('/')}` });
  }

  return (
    <>
      <StructuredData metaKey="vedas" />
      <PageLayout metaKey="vedas" title={title} breadcrumbs={crumbs} className="layout-md">
          ...
          <section>
            ...
            <ul className="space-y-2">
              {children.map((c: any, i: number) => {
                ...
                const href = `/vedas/${[...rawSegments, String(slug)].join('/')}`;
                return (
                  <li key={i}>
