...
export default function ItemClient({
  ...
  const breadcrumbs = [{
    ...
    label: chapterLabel,
    href: `/vedas/${slug}/${chapter}`
  }, {
    label: `${detailLabel} ${itemNum}`
  }];
  if (isLoading && !entry) {
    return <PageLayout metaKey="vedas" title="" breadcrumbs={breadcrumbs} className="layout-md">
        <div className="flex items-center justify-center py-12"><Loader /></div>
      </PageLayout>;
  }
  return <PageLayout metaKey="vedas" title={title} breadcrumbs={breadcrumbs} className="layout-md">
      ...
      {introduction && <section className="mt-8 rounded-2xl border border-[#d8a25a]/30 bg-[#fffaf3] p-5 md:p-6">
          <h2 className="text-sm font-extrabold uppercase tracking-widest text-[#a89278] mb-3">{vedas_page.introduc}</h2>
      ...
      {scriptureText && <section className="mt-6 rounded-2xl border border-[#edc98f]/50 bg-[#fffaf3] p-5 md:p-6">
          <h2 className="text-sm font-extrabold uppercase tracking-widest text-[#a89278] mb-3">{vedas_page.scriptur_text}</h2>
          <div className="text-base text-[#5b2d12] leading-relaxed"><Paragraphs text={scriptureText} /></div>
        </section>}

      {philosophicalExplanation && <section className="mt-6 rounded-2xl border border-[#e0a632]/30 bg-[#fffaf3] p-5 md:p-6">
          <h2 className="text-sm font-extrabold uppercase tracking-widest text-[#a89278] mb-3">{vedas_page.philosop_signific}</h2>
          <div className="text-base text-[#5b2d12] leading-relaxed"><Paragraphs text={philosophicalExplanation} /></div>
        </section>}

      <nav className="mt-8 flex items-center justify-between gap-4">
        {itemNum > 1 ? <Link href={`/vedas/${slug}/${chapter}/${item.replace(/\d+$/, String(itemNum - 1))}`} className="text-sm font-semibold text-[#7a2e1f] hover:text-[#92400e]">
            ← {detailLabel} {itemNum - 1}
          </Link> : <span />}

        <Link href={`/vedas/${slug}/${chapter}`} className="text-xs font-bold uppercase tracking-widest text-[#a89278] hover:text-[#92400e]"> {vedas_page.back} {chapterLabel}
        </Link>
