...
const page = {
  ...
  scriptur_teaching: "Scripture & Teachings",
...
const SLUG_CONFIG: Record<string, {
  ...
  rigveda: {
    icon: '🔥',
    fileKey: 'vedas_rigveda',
    accentFrom: 'from-[#7c2d12]',
    accentVia: 'via-[#c2410c]',
    accentTo: 'to-[#f59e0b]',
    textAccent: 'text-[#7c2d12]',
    borderAccent: 'border-[#c2410c]',
    chaptersKey: 'mandalas',
    chaptersLabel: 'The 10 Mandalas of Rigveda',
    itemLabel: 'Mandala',
    chapterPrefix: 'mandala',
    deitiesKey: 'major_rigvedic_deities',
    deitiesLabel: 'Major Rigvedic Deities'
  },
  yajurveda: {
    icon: '🪔',
    ...
    chaptersKey: 'yajurveda_chapters',
    chaptersLabel: 'Chapters of the Yajurveda',
    itemLabel: 'Chapter',
    chapterPrefix: 'chapter',
    deitiesKey: 'major_yajurvedic_deities',
    deitiesLabel: 'Major Yajurvedic Deities'
  },
  samaveda: {
    icon: '🎵',
    ...
    chaptersKey: 'samaveda_sections',
    chaptersLabel: 'Hymns of the Samaveda',
    itemLabel: 'Hymn',
    chapterPrefix: 'hymn',
    deitiesKey: 'major_samavedic_deities',
    deitiesLabel: 'Major Samavedic Deities'
  },
  atharvaveda: {
    icon: '🌿',
    ...
    chaptersKey: 'atharvaveda_books',
    chaptersLabel: 'Books of the Atharvaveda',
    itemLabel: 'Book',
    chapterPrefix: 'book',
    deitiesKey: 'major_atharvavedic_deities',
    deitiesLabel: 'Major Atharvavedic Deities'
  }
};
...
function ScriptureSection({
  ...
  return <div className={`relative overflow-hidden rounded-3xl border border-[#d8a25a]/30 p-6 md:p-8 ${shells[index % 3]} shadow-[0_8px_30px_rgba(146,64,14,0.06)]`}>
      {/* Top accent bar */}
  ...
  return <div className={`rounded-2xl border border-[#d8a25a]/30 overflow-hidden ${shells[index % 3]} transition-shadow duration-300 hover:shadow-[0_8px_30px_rgba(146,64,14,0.08)]`}>
      <button type="button" onClick={() => setOpen(!open)} className="w-full flex items-center gap-4 p-4 md:p-5 text-left cursor-pointer">
        ...
        <div className="flex-1 min-w-0">
          ...
          {totalHymns > 0 && <p className="text-xs text-[#6b5d4f] mt-0.5">{totalHymns} {page.hymns}</p>}
        </div>
        {hasExpandableContent ? <svg className={`h-4 w-4 text-[#b45309] shrink-0 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" />
          </svg> : <Link href={chapterHref} className="text-xs font-bold text-[#b45309] hover:text-[#92400e] transition-colors"> {page.view} </Link>}
      </button>

      {open && hasExpandableContent && <div className="px-4 md:px-5 pb-5 pt-0 border-t border-[#edc98f]/40 space-y-4">
          {intro && <div className="mt-4">
              <p className="text-sm text-[#5b2d12] leading-relaxed">{intro}</p>
            </div>}

          {scriptureText && <div className="rounded-xl border border-[#edc98f]/50 bg-[#fffaf3] p-4">
          ...
          {philoExplanation && <div className="rounded-xl border border-[#e0a632]/30 bg-[#fffaf3] p-4">
              <h5 className="text-[10px] font-bold uppercase tracking-widest text-[#a89278] mb-2">{page.philosop_explanat}</h5>
              <div className="text-sm text-[#5b2d12] leading-relaxed">
          ...
          <div className="text-right">
            <Link href={chapterHref} className="inline-flex items-center gap-1.5 text-sm font-bold text-[#b45309] hover:text-[#92400e] transition-colors"> {page.view_1} {itemLabel} {num} {page.details} <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default function SlugClient({
  ...
  if (isLoading && !ns?.title) {
    return <PageLayout metaKey={cfg.fileKey} title="" breadcrumbs={breadcrumbs} className="layout-md">
        <div className="flex items-center justify-center py-12"><Loader /></div>
      </PageLayout>;
  }
  return <PageLayout metaKey={cfg.fileKey} title={title} breadcrumbs={breadcrumbs} className="layout-md">
      ...
      <section className="relative overflow-hidden rounded-3xl border border-[#d8a25a]/30 bg-linear-to-br from-[#fffaf3] via-[#fdf0d7] to-[#fff8ef] p-6 md:p-10">
        ...
        <div className="relative z-10">
          ...
          {description && <p className="text-lg md:text-xl text-[#5b2d12] leading-relaxed italic font-medium">{description}</p>}
        </div>
      </section>
      ...
      {introduction && <>
          ...
          <section className="rounded-3xl border border-[#d8a25a]/20 bg-[#fffaf0] p-6 md:p-10 shadow-[0_8px_30px_rgba(146,64,14,0.06)]">
            <h2 className={`text-2xl md:text-3xl font-extrabold bg-linear-to-r ${cfg.accentFrom} ${cfg.accentVia} ${cfg.accentTo} bg-clip-text text-transparent mb-6`}> {page.introduc} </h2>
      ...
      {scriptureText.length > 0 && <>
          <OrnamentDivider />
          <section>
            <h2 className="text-2xl md:text-3xl font-extrabold bg-linear-to-r from-[#92400e] via-[#c2410c] to-[#ea580c] bg-clip-text text-transparent mb-8 text-center"> {page.scriptur_teaching} </h2>
            <div className="space-y-6">
              {scriptureText.map((item: Record<string, unknown>, i: number) => <ScriptureSection key={i} item={item} index={i} accentFrom={cfg.accentFrom} accentVia={cfg.accentVia} accentTo={cfg.accentTo} />)}
            </div>
          </section>
        </>}
      ...
      {philosophicalExplanation && <>
          <OrnamentDivider />
          <section className="rounded-3xl border border-[#d8a25a]/20 bg-[#fffaf0] p-6 md:p-10 shadow-[0_8px_30px_rgba(146,64,14,0.06)]">
            <h2 className={`text-2xl md:text-3xl font-extrabold bg-linear-to-r ${cfg.accentFrom} ${cfg.accentVia} ${cfg.accentTo} bg-clip-text text-transparent mb-6`}> {page.philosop_foundati} </h2>
            <div className="text-base text-[#5b2d12] leading-relaxed">
      ...
      {deities.length > 0 && <>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {deities.map((d: Record<string, unknown>, i: number) => <DeityCard key={i} deity={d} accentFrom={cfg.accentFrom} accentVia={cfg.accentVia} accentTo={cfg.accentTo} />)}
            </div>
          </section>
        </>}
      ...
      {chapters.length > 0 && <>
          ...
          <section>
            ...
            <div className="space-y-4">
              {chapters.map((item: Record<string, unknown>, i: number) => <ChapterRow key={i} item={item} index={i} itemLabel={cfg.itemLabel} accentFrom={cfg.accentFrom} accentTo={cfg.accentTo} slug={slug} chapterPrefix={cfg.chapterPrefix} />)}
            </div>
          </section>
        </>}
      ...
      {relatedConcepts.length > 0 && <>
          ...
          <section className="rounded-3xl border border-[#d8a25a]/20 bg-linear-to-br from-[#fffaf3] via-[#fef3e2] to-[#fbe8c8] p-6 md:p-8 text-center">
            <h2 className="text-xl font-extrabold text-[#3d2e22] mb-5">{page.related_concepts}</h2>
