        ...
        {page.story_divided_by_kandas && typeof page.story_divided_by_kandas === 'object' && (
          <section className="mb-12">
            <div className="relative mb-8">
              ...
              <p className="text-center text-gray-600 mt-4 text-md md:text-md max-w-2xl mx-auto">Experience the complete epic journey through seven magnificent chapters, each revealing profound wisdom and timeless values.</p>
            </div>
                ...
                {Object.entries(page.cultural_importance).map(([key, value]: [string, any], idx: number) => (
                  <div key={idx} className="flex gap-4">
                    <div className="flex-shrink-0 w-10 h-10 bg-amber-500 rounded-full flex items-center justify-center shadow-md">
                      <span className="text-white text-md md:text-md">✦</span>
                    </div>
                    <div className="flex-1">
                      <h3 className="text-md font-bold text-amber-900 mb-2 capitalize">{key.replace(/_/g, ' ')}</h3>
                      <p className="text-md md:text-md text-gray-700 leading-relaxed">{value}</p>
                    </div>
                  </div>
                ))}
              </div>
