...
const ns = useLocaleSection('scriptures_itihasas');
const __getLoc = (p: string) => {
  if (!ns) return '';
  const parts = p.split('.');
  const namespaceKey = parts[0] === 'scriptures_itihasas' ? parts.shift() : 'scriptures_itihasas';
...
export default function Page({ searchParams }: any) {
  ...
  const page: any = (() => {
    ...
    return {
      title: typeof k.title === 'string' ? k.title : (__getLoc('scriptures_itihasas.title') || ''),
      list: Array.isArray(k.list) ? k.list : parseList(__getLoc('scriptures_itihasas.list'))
    };
  })();
  return (
    <>
