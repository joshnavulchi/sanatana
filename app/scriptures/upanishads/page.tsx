      ...
      >
        <header className="rounded-lg overflow-hidden mb-6 border p-6 bg-gradient-to-r from-indigo-50 via-teal-50 to-emerald-50">
          <div className="max-w-4xl mx-auto">
            ...
            <p className="mt-3 text-xl md:text-lg text-indigo-700">{page.purpose}</p>
          </div>
        </header>
          ...
          <main className="lg:col-span-3 space-y-6">
            {(page.list || []).map((group: any, gi: number) => (
              <section key={gi} className="rounded-lg p-6 bg-white shadow-sm border-t-4 border-indigo-200">
                {group.category ? <h3 className="text-xl md:text-lg font-semibold mb-3 text-indigo-800">{group.category}</h3> : null}
                    ...
                    <li key={idx} className="border rounded-md p-3 bg-gradient-to-r from-white to-indigo-50">
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <p className="text-base md:text-md md:text-sm text-slate-600">{it.summary}</p>
                        </div>
                        <div className="text-base md:text-md text-amber-700 ml-4 px-2 py-1 bg-amber-100 rounded-full">{it.veda}</div>
                      </div>
                    </li>
                  ))}
                </ul>
          ...
          <aside className="space-y-4">
            ...
            {page.unique_insights ? (
              <div className="p-4 rounded-lg bg-indigo-50 border-l-4 border-indigo-400">
                ...
                <p className="text-base md:text-md md:text-sm">{page.unique_insights}</p>
              </div>
            ) : null}
          </aside>
        </div>
