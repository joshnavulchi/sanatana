        ...
        {page.introduction && (
          <section className="mb-12">
            ...
            <div className="bg-white border-l-4 border-blue-500 shadow-lg rounded-r-xl p-6">
              <p className="text-xl md:text-lg md:text-md text-gray-700 leading-relaxed">{page.introduction}</p>
            </div>
          </section>
        )}
        {/* Major Characters */}
                  ...
                  <div className="relative p-6">
                    <div className="flex items-start gap-3 mb-3">
                      ...
                      <div className="flex-1">
                        <h3 className="text-xl md:text-lg md:text-md font-bold text-blue-900 group-hover:text-indigo-600 transition-colors">{c.name}</h3>
                      </div>
                    </div>
                    {c.description && (
                      <p className="text-xl md:text-lg md:text-md text-gray-700 leading-relaxed ">{c.description}</p>
                    )}
                  </div>
                </div>
              ))}
                  ...
                  <div className="pl-8 pr-6 py-5">
                    <div className="flex items-start gap-4">
                      <div className="flex-shrink-0 w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center border-2 border-blue-400">
                        <span className="text-xl md:text-lg md:text-md font-bold text-blue-700">{idx + 1}</span>
                      </div>
                      <div className="flex-1">
                        <h3 className="text-xl md:text-lg md:text-md font-bold text-blue-900 mb-2">{typeof ct.title === 'string' ? ct.title : ct['title']}</h3>
                        {ct.description && <p className="text-xl md:text-lg md:text-md text-gray-700 leading-relaxed">{ct.description}</p>}
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {page.parvas && Object.keys(page.parvas).length > 0 && (
          <section className="mb-12">
            <div className="relative mb-8">
              ...
              <p className="text-center text-gray-600 mt-4 text-xl md:text-lg md:text-md max-w-2xl mx-auto">Journey through the eighteen books of the Mahabharata, each revealing profound wisdom about duty, morality, and the complexity of human nature.</p>
            </div>
                        ...
                        {parva.detailednarration && (
                          <div className="mb-6 pl-0 md:pl-22">
                            <div className="bg-white bg-opacity-70 rounded-xl p-5 border border-gray-200">
                              <p className="text-lg md:text-md text-gray-800 leading-relaxed">
                        ...
                        {parva.moralpsychologicalphilosophicallessons && (
                          <div className="mb-6 pl-0 md:pl-22">
                            <div className="bg-white bg-opacity-90 rounded-xl p-5 border-l-4 border-blue-500">
                              <p className="text-lg md:text-md font-bold text-blue-700 uppercase tracking-wider mb-2">📚 Key Lessons</p>
                              <p className="text-lg md:text-md text-gray-700 leading-relaxed">{parva.moralpsychologicalphilosophicallessons}</p>
                            </div>
                          </div>
                        )}
        ...
        {page.conclusion && (
          <section className="mb-12">
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl border-2 border-blue-300 shadow-xl p-8">
              <p className="text-xl md:text-lg md:text-md text-gray-700 leading-relaxed">{page.conclusion}</p>
            </div>
          </section>
        )}
      </PageLayout>
