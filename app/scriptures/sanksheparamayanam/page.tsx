  ...
  return (
    <>
      ...
      >
        ...
        <p className="mb-6 text-xl md:text-lg text-center text-slate-600">
        ...
        <div className="space-y-8">
                ...
                {page.main_characters.map((c: any, idx: number) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      {c.role ? <div className="text-xl md:text-lg text-slate-600">{c.role}</div> : null}
                      {c.note ? <div className="mt-1 text-base md:text-md md:text-sm text-slate-500">{c.note}</div> : null}
                    </div>
                  </article>
                ))}
              </div>
          ...
          {page.important_places && page.important_places.length > 0 && (
            <section className="">
              <h4 className="text-xl md:text-lg font-semibold text-amber-800 mb-3">Important Places</h4>
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                {page.important_places.map((p: any, idx: number) => (
                  <div key={idx} className="p-4 bg-white shadow-sm rounded-md border border-slate-100">
                    <div className="text-xl md:text-lg font-semibold text-amber-500">{p.name}</div>
                    {p.desc ? <div className="text-xl md:text-lg text-slate-600 mt-1">{p.desc}</div> : null}
                  </div>
                ))}
              </div>
            </section>
          ...
          {page.timeline && page.timeline.length > 0 && (
            <section className="bg-white border border-slate-100 rounded-lg shadow-sm">
              <h6 className="text-xl md:text-lg font-semibold text-slate-800 mb-4">Timeline</h6>
                ...
                {page.timeline.map((ev: any, idx: number) => (
                  <li key={idx} className="flex gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      {ev.desc ? <div className="text-xl md:text-lg text-slate-600 mt-1">{ev.desc}</div> : null}
                    </div>
                  </li>
                ))}
              </ol>
          ...
          {page.core_themes && page.core_themes.length > 0 && (
            <section>
              <p className="text-xl md:text-lg font-semibold text-amber-800 mb-3">Core Themes</p>
              <div className="flex flex-wrap gap-2">
                {page.core_themes.map((ct: any, idx: number) => (
                  <span
                    key={idx}
                    className="inline-flex items-center px-4 py-2 rounded-full bg-white border border-slate-100 text-xl md:text-lg text-slate-700 shadow-sm"
                  >
