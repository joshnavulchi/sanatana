                ...
                {page.main_characters.map((c: any, idx: number) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      {c.role ? <div className="text-xl text-slate-600">{c.role}</div> : null}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-8">
                {page.important_places.map((p: any, idx: number) => (
                  <div key={idx} className="p-4 bg-white shadow-sm rounded-md border border-slate-100">
                    ...
                    {p.desc ? <div className="text-xl text-slate-600 mt-1">{p.desc}</div> : null}
                  </div>
                ))}
              </div>
            </section>
                ...
                {page.timeline.map((ev: any, idx: number) => (
                  <li key={idx} className="flex gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      {ev.desc ? <div className="text-xl text-slate-600 mt-1">{ev.desc}</div> : null}
                    </div>
                  </li>
                ))}
              </ol>
            ...
            <section>
              ...
              <div className="flex flex-wrap gap-3">
                {page.core_themes.map((ct: any, idx: number) => (
                  <span
                    key={idx}
                    className="inline-flex items-center px-3 py-1 rounded-full bg-white border border-slate-100 text-xl text-slate-700 shadow-sm"
                  >
