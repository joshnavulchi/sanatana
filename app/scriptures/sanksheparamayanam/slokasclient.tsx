    ...
    <section className="max-w-7xl mx-auto">
      <header className="mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <span className="text-md md:text-lg px-3 py-2 bg-amber-50 rounded-full text-amber-700 shadow-sm">
            {total} slokas
          </span>
        </div>
      </header>
            ...
            >
              ...
              <div className="flex justify-between items-start">
                ...
                <div className="ml-4 flex flex-col items-end gap-2">
                  ...
                  <span className="text-md md:text-lg text-slate-700">{idx + 1}</span>
                </div>
              </div>

              <div className="mt-4">
                {context ? (
                  <div className="text-md md:text-lg text-slate-700 leading-relaxed">
                    <p className={`${isExpanded ? "" : "line-clamp-[8]"}`}>{isExpanded ? context : previewContext}</p>
                    {context.length > 220 ? (
                      <button
                        onClick={() => toggleExpand(idx)}
                        className="mt-3 inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-base bg-amber-50 text-amber-700 border border-amber-100 shadow-sm hover:brightness-105 transition"
                  ...
                  <p className="text-md md:text-lg text-slate-700">No context available.</p>
                )}
              </div>

              <div className="mt-4">
                {meaning ? (
                  <div className="text-md md:text-lg text-slate-400 leading-relaxed">
                    <p className={`${isExpanded ? "" : "line-clamp-[8]"}`}>{isExpanded ? meaning : previewMeaning}</p>
                    {meaning.length > 220 ? (
                      <button
                        onClick={() => toggleExpand(idx)}
                        className="mt-3 inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-md md:text-lg bg-amber-50 text-amber-700 border border-amber-100 shadow-sm hover:brightness-105 transition"
                        aria-expanded={isExpanded}
                      >
                        {isExpanded ? "Show less" : "Read more"}
                        <svg className={`w-4 h-4 transition-transform ${isExpanded ? "rotate-180" : ""}`} viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  ...
                  <p className="text-md md:text-lg text-slate-400">No meaning available.</p>
                )}
              </div>
      ...
      <div className="mt-10 flex justify-center">
        <button
          ...
          className={`cursor-pointer px-6 py-3 rounded-full text-md md:text-lg shadow-xl transition transform hover:-translate-y-0.5 ${visible >= maxVisible
