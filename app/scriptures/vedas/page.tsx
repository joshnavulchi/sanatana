      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {(page.structure || []).map((v: any, idx: number) => (
                  <article key={idx} className="border-l-4 border-emerald-400 rounded-lg p-4 bg-emerald-50">
                    ...
                    <p className="text-xs md:base-sm mb-2 text-emerald-700">{v.content || v.Content}</p>
                    ...
                    <li key={i} className="bg-white border rounded-md p-3 shadow-sm">
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          {u.summary ? <p className="text-xs md:base-sm text-slate-600">{u.summary}</p> : null}
                        </div>
          ...
          <aside className="md:col-span-1 space-y-4">
            <div className="p-4 rounded-lg bg-yellow-50 border-l-4 border-amber-400">
              ...
              <p className="text-xs md:base-sm"><strong>Meaning:</strong> {page.origin.meaning || ''}</p>
              <p className="text-xs md:base-sm"><strong>Period:</strong> {page.origin.period || ''}</p>
              <p className="text-xs md:base-sm"><strong>Transmission:</strong> {page.origin.transmission || ''}</p>
            </div>
            ...
            <div className="p-4 rounded-lg bg-sky-50 border-l-4 border-sky-400">
              ...
              <p className="text-xs md:base-sm"><strong>Nature:</strong> {page.authorship.nature || ''}</p>
              <p className="text-xs md:base-sm"><strong>Process:</strong> {page.authorship.process || ''}</p>
              <p className="text-xs md:base-sm"><strong>Compiler:</strong> {page.authorship.compiler || ''}</p>
            </div>
            ...
            <div className="p-4 rounded-lg bg-emerald-50 border-l-4 border-emerald-400">
              ...
              <p className="text-xs md:base-sm">{page.language}</p>
            </div>
        ...
        {(page.structure || []).map((item: any, i: number) => (
          <div key={i} className="mb-4 p-4 bg-white border rounded-lg shadow-sm">
            ...
            <p className="text-xs md:base-sm text-slate-700"><b>Content: </b>{item.content || item.Content}</p>
            <p className="text-xs md:base-sm text-slate-600"><b>Features: </b>{item.features || item.Features}</p>
          </div>
        ))}
              ...
              {page.timeline.map((ev: any, i: number) => (
                <li key={i} className="mb-4 relative">
                  ...
                  <div className="bg-white p-3 rounded-md border shadow-sm">
                    <div className="text-xs md:base-sm text-slate-800 font-semibold">{ev.title || ev.name || ev.event}</div>
                    {ev.period ? <div className="text-base text-slate-600">{ev.period}</div> : null}
                    {ev.description ? <div className="text-xs md:base-sm text-slate-700 mt-1">{ev.description}</div> : null}
                  </div>
                </li>
              ))}
            </ol>
