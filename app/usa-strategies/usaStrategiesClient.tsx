...
export default function UsaStrategiesClient() {
  ...
  const data: UsaStrategiesData = {
    ...
    timeline_by_president: Array.isArray(ns?.timeline_by_president)
      ? ns.timeline_by_president
      : [],
    cross_cutting_tools: ns?.cross_cutting_tools,
    interaction_patterns: Array.isArray(ns?.interaction_patterns)
      ? ns.interaction_patterns
      : [],
    strategies_effectiveness: Array.isArray(ns?.strategies_effectiveness)
      ? ns.strategies_effectiveness
      : [],
    ...
    asia_interventions_timeline: Array.isArray(ns?.asia_interventions_timeline)
      ? ns.asia_interventions_timeline
      : [],
    map_targeted_regions: ns?.map_targeted_regions,
    map_visualization: ns?.map_visualization,
    datasets: ns?.datasets,
    forced_surrenders: Array.isArray(ns?.forced_surrenders)
      ? ns.forced_surrenders
      : [],
    additional_forced_surrenders_not_directly_to_us: Array.isArray(
      ns?.additional_forced_surrenders_not_directly_to_us
    )
      ? ns.additional_forced_surrenders_not_directly_to_us
      : [],
  ...
  if (isLoading && !data.title) {
    return (
      <PageLayout
        metaKey="usa-strategies"
        title={data.title}
        description={data.description}
        breadcrumbs={[{ labelKey: 'Home', href: '/' }, { label: 'USA Strategies' }]}
        className="layout-md"
      >
  ...
  const quickStats = [
    { label: 'Presidential Eras', value: data.timeline_by_president.length },
    { label: 'Cross-Region Patterns', value: data.interaction_patterns.length },
    { label: 'Map Points', value: data.datasets?.map_points?.length ?? 0 },
    { label: 'Surrender Records', value: data.forced_surrenders.length },
  ];

  const sectionCardClass =
    'scroll-mt-24 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8';

  const flowHighlights = Array.from(
    new Set(
    ...
    >
      <div className="space-y-8">
        <section className="relative overflow-hidden rounded-3xl border border-sky-200 bg-gradient-to-br from-sky-50 via-white to-violet-50 p-6 shadow-sm md:p-8">
          <div
            className="absolute -right-16 -top-16 h-44 w-44 rounded-full bg-sky-100 blur-3xl"
            aria-hidden="true"
          />
          <div
            className="absolute -bottom-14 -left-12 h-36 w-36 rounded-full bg-violet-100 blur-3xl"
            aria-hidden="true"
          />
          <div className="relative">
            <span className="inline-flex rounded-full border border-sky-200 bg-white px-3 py-1 text-base font-semibold uppercase tracking-wider text-sky-700">
              USA Strategies Atlas
            </span>
            <h3 className="mt-4 text-2xl font-bold tracking-tight text-slate-900 md:text-5xl">
              {data.title}
            </h3>
            {data.description ? (
              <p className="mt-4 max-w-5xl text-md md:text-lg leading-7 text-slate-700 md:text-base">
                {data.description}
              </p>
            ) : null}
        ...
        <div className="space-y-6">
          {data.flowchart?.content ? (
            <section
              id="flowchart"
              className="scroll-mt-24 rounded-3xl border border-cyan-200 bg-gradient-to-br from-cyan-50 via-white to-sky-50 p-6 shadow-sm md:p-8"
            >
              <div className="flex flex-wrap items-center gap-2">
                <span className="inline-flex rounded-full border border-cyan-300 bg-white px-3 py-1 text-xs font-semibold uppercase tracking-wider text-cyan-700">
                  Flow Architecture
                </span>
                {data.flowchart.file ? (
              ...
              <h5 className="mt-3 text-2xl font-bold text-slate-900">Strategic Flowchart (Redesigned)</h5>
              <p className="mt-2 text-base text-slate-700">
                Reality-first pipeline from objective setting to policy outcomes, extracted from source
                Mermaid graph.
              </p>

              {flowHighlights.length ? (
                <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                  {flowHighlights.map((node) => (
                    <div
                      key={node}
                      className="rounded-xl border border-cyan-200 bg-white px-3 py-2 text-sm font-medium text-slate-700"
                    >
                      {node}
                    </div>
                  ))}
          ...
          {data.timeline_by_president.length ? (
            <section id="timeline" className={sectionCardClass}>
              <h5 className="text-2xl font-bold text-slate-900">Timeline by President</h5>
              <div className="mt-5 grid gap-4">
                {data.timeline_by_president.map((entry, index) => (
                  <article
                    key={`${entry.president ?? 'president'}-${index}`}
                  ...
                  >
                    ...
                    {entry.key_strategies?.length ? (
                      <ul className="mt-3 list-disc pl-5 text-md md:text-lg text-slate-700">
                    ...
                    {entry.tools_used?.length ? (
                      <p className="mt-2 text-base text-slate-600">
                        Tools: {entry.tools_used.join(', ')}
                      </p>
                    ) : null}
                    {entry.notable_policies_events?.length ? (
                      <ul className="mt-2 list-disc pl-5 text-base text-slate-600">
                        {entry.notable_policies_events.map((item, itemIndex) => (
                          <li key={`${entry.president}-event-${itemIndex}`}>{item}</li>
                        ))}
                      </ul>
                    ) : null}
          ...
          {data.cross_cutting_tools ? (
            <section id="tools" className={sectionCardClass}>
              <h5 className="text-2xl font-bold text-slate-900">Cross-Cutting Tools</h5>
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                {(
                  [
                    ['finance', data.cross_cutting_tools.finance],
                    ['technology_controls', data.cross_cutting_tools.technology_controls],
                    ['machines_security', data.cross_cutting_tools.machines_security],
                  ] as const
                ).map(([name, group]) =>
                  group ? (
                    <article
                    ...
                    >
                      ...
                      <div className="mt-3 space-y-3">
                        {Object.entries(group).map(([itemKey, itemValue]) => (
                          <div key={itemKey} className="rounded-xl border border-slate-200 bg-white p-4">
                            ...
                            {itemValue.description ? (
                              <p className="mt-1 text-md md:text-lg text-slate-700">
                                {itemValue.description}
                              </p>
                            ) : null}
          ...
          {data.interaction_patterns.length ? (
            <section id="patterns" className={sectionCardClass}>
              <h5 className="text-2xl font-bold text-slate-900">Interaction Patterns</h5>
              <div className="mt-5 grid gap-4 md:grid-cols-3">
                {data.interaction_patterns.map((entry, index) => (
                  <article
          ...
          {data.strategies_effectiveness.length ? (
            <section id="effectiveness" className={sectionCardClass}>
              <div className="flex flex-wrap items-center justify-between gap-3">
                <h5 className="text-2xl font-bold text-slate-900">Reality Chart</h5>
                <span className="inline-flex rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-slate-700">
                  Derived from strategies_effectiveness
                </span>
              </div>
              <p className="mt-2 text-base text-slate-600">
                Comparative view of real-world strategic leverage using qualitative effectiveness converted
              ...
              <div className="mt-5 space-y-4">
                {data.strategies_effectiveness.map((entry, index) => {
                  ...
                  return (
                    <article
                      key={`${entry.strategy ?? 'strategy'}-${index}`}
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <span
                          className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold uppercase tracking-wide ${tone.badge}`}
                        >
                          {tone.label} • {score}%
                        </span>
                      </div>
                      ...
                      {entry.evidence?.length ? (
                        <p className="mt-3 text-sm text-slate-500">
                          Evidence refs ({entry.evidence.length}): {entry.evidence.join(', ')}
                        </p>
                      ) : null}
                    </article>
                  );
          ...
          {data.us_targeting_motives ? (
            <section id="motives" className={sectionCardClass}>
              <h5 className="text-2xl font-bold text-slate-900">Targeting Motives</h5>
              <div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          {data.comparison_latin_america_vs_asia ? (
            <section id="comparison" className={sectionCardClass}>
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                {(
                  [
                    ['Latin America', data.comparison_latin_america_vs_asia.latin_america],
                    ['Asia Pacific', data.comparison_latin_america_vs_asia.asia_pacific],
                  ] as const
                ).map(([label, section]) =>
                  section ? (
                    <article
          ...
          {data.regional_strategy_comparison_v3?.regions?.length ? (
            <section id="regional" className={sectionCardClass}>
              <h5 className="text-2xl font-bold text-slate-900">Regional Comparison</h5>
              {data.regional_strategy_comparison_v3.last_updated_utc ? (
                <p className="mt-2 text-base text-slate-600">
                  Last updated: {data.regional_strategy_comparison_v3.last_updated_utc}
                </p>
              ) : null}
              <div className="mt-5 grid gap-4">
                {data.regional_strategy_comparison_v3.regions.map((region, index) => (
                  <article
                  ...
                  >
                    ...
                    <div className="mt-3 grid gap-3 md:grid-cols-2">
                      {(region.top_strategies ?? []).map((strategy, strategyIndex) => (
                        <div
                          key={`${region.region}-top-strategy-${strategyIndex}`}
                        ...
                        >
                          ...
                          {strategy.why ? (
                            <p className="mt-1 text-md md:text-lg text-slate-700">{strategy.why}</p>
                          ) : null}
                        </div>
                      ))}
                    </div>
          ...
          {data.asia_interventions_timeline.length ? (
            <section id="interventions" className={sectionCardClass}>
              <h5 className="text-2xl font-bold text-slate-900">Asia Interventions Timeline</h5>
              <div className="mt-5 grid gap-4">
                {data.asia_interventions_timeline.map((entry, index) => (
                  <article
          ...
          {data.map_targeted_regions ? (
            <section id="map-regions" className={sectionCardClass}>
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                {(
                  [
                    ['Latin America Layer', data.map_targeted_regions.latin_america_layer],
                    ['Asia Pacific Layer', data.map_targeted_regions.asia_pacific_layer],
                  ] as const
                ).map(([label, layer]) =>
                  layer ? (
                    <article
                    ...
                    >
                      <h6 className="text-md md:text-lg font-semibold text-slate-900">{label}</h6>
                      {layer.label ? (
                        <p className="mt-1 text-md md:text-lg text-slate-700">{layer.label}</p>
                      ) : null}
                      {layer.countries?.length ? (
                        <p className="mt-2 text-base text-slate-600">
                          Countries: {layer.countries.join(', ')}
                        </p>
                      ) : null}
                      {layer.explanatory_note ? (
                        <p className="mt-2 text-base text-slate-600">{layer.explanatory_note}</p>
                      ) : null}
          ...
          {data.datasets?.map_points?.length ? (
            <section id="d3-map" className={sectionCardClass}>
              <div className="flex flex-wrap items-center gap-2">
              ...
              <h5 className="mt-3 text-2xl font-bold text-slate-900">Global Strategy Map</h5>

              <UsaStrategyD3Map
                points={data.datasets.map_points}
                mapVisualization={data.map_visualization}
                className="mt-4"
              />

              {data.map_targeted_regions?.projection_hint ? (
                <p className="mt-4 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 md:text-base">
                  Projection hint: {data.map_targeted_regions.projection_hint}
                </p>
              ) : null}
                ...
                <table className="min-w-full divide-y divide-slate-200 text-md md:text-lg">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-left font-semibold text-slate-700">ISO3</th>
                  ...
                  <tbody className="divide-y divide-slate-100 bg-white">
                    {data.datasets.map_points.map((point, index) => (
                      <tr key={`${point.iso3 ?? 'point'}-${index}`}>
                ...
                <table className="min-w-full divide-y divide-slate-200 text-md md:text-lg">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-left font-semibold text-slate-700">Why</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 bg-white">
                    {data.forced_surrenders.map((entry, index) => (
                      <tr key={`${entry.country ?? 'surrender'}-${index}`}>
          ...
          {data.additional_forced_surrenders_not_directly_to_us.length ? (
            <section id="additional-surrenders" className={sectionCardClass}>
              ...
              <div className="mt-4 overflow-x-auto rounded-2xl border border-slate-200">
                <table className="min-w-full divide-y divide-slate-200 text-md md:text-lg">
                  ...
                  <tbody className="divide-y divide-slate-100 bg-white">
                    {data.additional_forced_surrenders_not_directly_to_us.map((entry, index) => (
                      <tr key={`${entry.country ?? 'additional-surrender'}-${index}`}>
                        <td className="px-3 py-2 text-slate-700">{entry.country}</td>
                        <td className="px-3 py-2 text-slate-700">{entry.when}</td>
                        <td className="px-3 py-2 text-slate-700">{entry.why}</td>
                      </tr>
