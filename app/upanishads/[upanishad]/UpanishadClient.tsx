...
export default function UpanishadsClient({ initialData, initialLocale, segments }: Props) {
  ...
  if (loading) {
    return (
      <PageLayout metaKey={metaKey} title={title} description={description} breadcrumbs={breadcrumbs} className="layout-md">
        <div className="flex items-center justify-center py-8 text-base sm:text-lg leading-relaxed font-normal"><Loader /></div>
      </PageLayout>
    );
  }
  ...
  if (error || !data) {
    return (
      <PageLayout metaKey={metaKey} title={title} description={description} breadcrumbs={breadcrumbs} className="layout-md">
        <div className="py-12 text-center text-base sm:text-lg leading-relaxed font-normal">{error || 'Content not available.'}</div>
      </PageLayout>
    );
  }
  ...
  return (
    <PageLayout metaKey={metaKey} title={title} description={description} breadcrumbs={breadcrumbs} className="layout-md">
      ...
      {typeof data?.introduction === 'string' && (
        <section>
          <p className="text-base sm:text-lg leading-relaxed mb-4 font-normal bg-gradient-to-r from-[#f0fdfa] via-[#a7f3d0]/30 to-[#f3e8ff]/10 rounded-xl px-3 py-2 shadow-sm animate-fadeInUp">{data.introduction}</p>
        </section>
      )}
      {/* Sections array */}
          ...
          {data.section.map((item: any, idx: number) => {
            ...
            return (
              <section key={idx}>
                ...
                {secContent ? (
                  <p className="text-base sm:text-lg leading-relaxed mb-4 font-normal bg-gradient-to-r from-[#f0fdfa] via-[#a7f3d0]/30 to-[#f3e8ff]/10 rounded-xl px-3 py-2 shadow-sm animate-fadeInUp">{secContent}</p>
                ) : null}
              </section>
            );
          })}
          ...
          {data.scripture_text.map((item: any, i: number) => {
            ...
            return (
              <section key={`scripture-${i}`}>
                ...
                {sContent ? (
                  <p className="text-base sm:text-lg leading-relaxed mb-4 font-normal bg-gradient-to-r from-[#f0fdfa] via-[#a7f3d0]/30 to-[#f3e8ff]/10 rounded-xl px-3 py-2 shadow-sm animate-fadeInUp">{sContent}</p>
                ) : null}
              </section>
            );
          })}
      ...
      {Array.isArray(data?.chapters) && data.chapters.length > 0 && (
        <section className="mt-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-base sm:text-lg leading-relaxed font-normal">
            {data.chapters.map((c: any, i: number) => (
              <a key={`chapter-${i}`} href={c?.path || '#'} className="block rounded-lg p-4 bg-gradient-to-br from-[#f0fdfa] via-[#a7f3d0]/30 to-[#f3e8ff]/10 hover:shadow-xl transition-all duration-300">
                <div className="text-base sm:text-lg leading-relaxed font-normal font-semibold text-[#3b3270]">{String(c?.title ?? `Chapter ${c?.chapter ?? i + 1}`)}</div>
                {c?.chapter && <div className="text-base sm:text-lg leading-relaxed font-normal">Chapter {String(c.chapter)}</div>}
                {c?.verses && Array.isArray(c.verses) && (
                  <div className="text-base sm:text-lg leading-relaxed font-normal">{String(c.verses.length)} verses</div>
                )}
              </a>
            ))}
          </div>
      ...
      {Array.isArray(data?.major_rigvedic_deities) && data.major_rigvedic_deities.length > 0 && (
        <section className="mt-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-base sm:text-lg leading-relaxed font-normal">
            {data.major_rigvedic_deities.map((d: any, i: number) => (
              <div key={`deity-${i}`} className="rounded-lg p-4 bg-gradient-to-br from-[#f0fdfa] via-[#a7f3d0]/30 to-[#f3e8ff]/10 text-base sm:text-lg leading-relaxed font-normal shadow-md">
                <h3 className="text-xl font-extrabold text-[#3b3270] leading-snug mb-2 animate-gradient-x">{String(d?.name || '')}</h3>
                {d?.role && <div className="text-base sm:text-lg leading-relaxed font-normal">{d.role}</div>}
                {d?.importance && <p className="text-base sm:text-lg leading-relaxed mb-4 font-normal">{d.importance}</p>}
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-base sm:text-lg leading-relaxed font-normal">
            {data.mandalas.map((m: any, i: number) => (
              <a key={`mandala-${i}`} href={m?.path || '#'} className="block rounded-lg p-4 bg-gradient-to-br from-[#f0fdfa] via-[#a7f3d0]/30 to-[#f3e8ff]/10 hover:shadow-xl transition-all duration-300">
                <div className="flex items-baseline justify-between text-base sm:text-lg leading-relaxed font-normal">
                  <div className="text-[#8b6914] text-base sm:text-lg leading-relaxed font-semibold">Mandala {String(m?.mandala ?? m?.number ?? i + 1)}</div>
                  <div className="text-base sm:text-lg leading-relaxed font-normal">{String(m?.hymn_count ?? '')} hymns</div>
                </div>
                {m?.hymns && Array.isArray(m.hymns) && m.hymns.length > 0 && (
                  <p className="text-base sm:text-lg leading-relaxed mb-4 font-normal">Example: {String(m.hymns[0]?.title || '')}</p>
                )}
              </a>
            ))}
          </div>
      ...
      {Array.isArray(data?.related_concepts) && data.related_concepts.length > 0 && (
        <section className="mt-6">
          ...
          <div className="flex flex-wrap gap-2 text-base sm:text-lg leading-relaxed font-normal">
            {data.related_concepts.map((c: any, i: number) => (
              <span key={`concept-${i}`} className="inline-flex items-center px-3 py-1 rounded-full bg-gradient-to-br from-[#f0fdfa] via-[#a7f3d0]/30 to-[#f3e8ff]/10 text-[#3b3270] text-base sm:text-lg leading-relaxed font-normal shadow-sm">{String(c)}</span>
            ))}
          </div>
        </section>
      )}
        ...
        if (val) {
          return (
            <section>
              ...
              <p className="text-base sm:text-lg leading-relaxed mb-4 font-normal bg-gradient-to-r from-[#f0fdfa] via-[#a7f3d0]/30 to-[#f3e8ff]/10 rounded-xl px-3 py-2 shadow-sm animate-fadeInUp">{val}</p>
            </section>
          );
        }
        return null;
