...
function normalizeNav(nav: unknown, basePath: string): NavLink[] {
  if (nav && typeof nav === 'object' && !Array.isArray(nav)) {
    return Object.entries(nav as Record<string, unknown>).filter(([, val]) => typeof val === 'string').map(([key, val]) => ({
      href: `${basePath}/${key}`,
      label: val as string
    }));
  }
...
export default function UpanishadsClient() {
  ...
  const scriptureSections = Array.isArray(pageNs?.scripture_text) ? (pageNs.scripture_text as unknown[]).filter(v => v && typeof v === 'object') as Record<string, unknown>[] : [];
  if (isLoading && !section) {
    return <PageLayout metaKey="upanishads" title="" breadcrumbs={[{
      label: 'Home',
      href: '/'
    }, {
      label: 'Upanishads'
  ...
  return <PageLayout metaKey="upanishads" title={title} breadcrumbs={[{
    label: 'Home',
    href: '/'
  }, {
    label: title
      ...
      {introduction && <div className="relative px-4 md:px-6 py-4 md:py-10 bg-[#fffaf0] rounded-2xl border border-[#d8a25a]/30 overflow-hidden mb-8 shadow-[0_8px_30px_rgba(146,64,14,0.06)]">
          <h2 className="text-2xl font-extrabold text-[#3d2e22] mb-4">{upanishads_page.introduc}</h2>
      ...
      {scriptureSections.length > 0 && <div className="mt-8">
          <h2 className="text-2xl font-extrabold text-[#3d2e22] mb-6">{upanishads_page.overview}</h2>
      ...
      {philosophical && <div className="mt-10 relative overflow-hidden rounded-3xl border border-[#d8a25a]/30 p-4 md:p-8 bg-linear-to-br from-[#fffaf3] via-[#fdf1dc] to-[#f8e4c0] shadow-[0_8px_30px_rgba(146,64,14,0.06)]">
          ...
          <h2 className="text-2xl font-extrabold text-[#3d2e22] mb-4">{upanishads_page.philosop_explanat}</h2>
          <div className="text-base md:text-lg text-[#5b2d12] leading-relaxed">
