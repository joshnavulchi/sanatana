...
export default function SlugClient({
  ...
  if (isLoading) {
    return <PageLayout metaKey={`upanishads_${slug}`} title="" breadcrumbs={[{
      label: 'Home',
      href: '/'
    }, {
      label: 'Upanishads',
  ...
  return <PageLayout metaKey={`upanishads_${slug}`} title={displayTitle} breadcrumbs={[{
    label: 'Home',
    href: '/'
  }, {
    label: 'Upanishads',
    ...
    label: displayTitle
      ...
      {scriptureSections.length > 0 && <div className="mt-10">
          ...
          <div className="grid grid-cols-1 gap-6">
            {scriptureSections.map((item, idx) => <SectionCard key={idx} item={item} index={idx} accentFrom="from-[#92400e]" accentVia="via-[#c2410c]" accentTo="to-[#ea580c]" />)}
          </div>
        </div>}
