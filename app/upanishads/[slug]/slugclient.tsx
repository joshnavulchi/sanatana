...
export default function SlugClient({ slug }: { slug: string }) {
  ...
  if (isLoading) {
    return (
      <PageLayout metaKey={`upanishads_${slug}`} title="" breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Upanishads', href: '/upanishads' }, { label: displayTitle }]} className="layout-md">
  ...
  return (
    <PageLayout metaKey={`upanishads_${slug}`} title={displayTitle} breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Upanishads', href: '/upanishads' }, { label: displayTitle }]} className="layout-md">
      {description && (
