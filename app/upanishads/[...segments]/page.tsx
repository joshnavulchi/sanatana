...
export default async function Page({ params }: Props) {
  ...
  const segments = ['upanishads', ...rawSegments];
  ...
  const crumbs = [{ label: 'Home', href: '/' }, { label: 'Upanishads', href: '/upanishads' }];
  let accum: string[] = [];
  for (const s of rawSegments) {
    accum.push(s);
    crumbs.push({ label: s, href: `/upanishads/${accum.join('/')}` });
  }

  return (
    <>
      <StructuredData metaKey="upanishads" />
      <PageLayout metaKey="upanishads" title={title} breadcrumbs={crumbs} className="layout-md">
          ...
          <section>
            ...
            <ul className="space-y-2">
              {children.map((c: any, i: number) => {
                ...
                const href = `/upanishads/${[...rawSegments, String(slug)].join('/')}`;
                return (
                  <li key={i}>
