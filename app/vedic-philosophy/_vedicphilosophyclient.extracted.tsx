...
const vedic_philosophy_structure_page = {
...
export default function VedicPhilosophyClient() {
  ...
  const philosophical = typeof structure?.philosophical_explanation === 'string' ? structure.philosophical_explanation : '';
  const scriptureSections = Array.isArray(structure?.scripture_text) ? (structure.scripture_text as unknown[]).filter(v => v && typeof v === 'object') as Record<string, unknown>[] : [];
  const topics = Array.isArray(structure?.topics) ? structure.topics as unknown[] : [];
  const links: TopicLink[] = topics.map(topic => {
    ...
    return {
      ...
      label: topicTitle,
      description: typeof (topic as Record<string, unknown>).description === 'string' ? (topic as Record<string, unknown>).description as string : undefined,
      totalSubtopics: Number((topic as Record<string, unknown>).total_subtopics || 0)
    };
  }).filter(Boolean) as TopicLink[];
  if (isLoading && links.length === 0) {
    return <PageLayout metaKey="vedic_philosophy_structure" title="" breadcrumbs={[{
      label: 'Home',
      href: '/'
    }, {
  ...
  return <PageLayout metaKey="vedic_philosophy_structure" title={title} breadcrumbs={[{
    ...
    label: title
      ...
      {introduction && <div className="relative px-4 md:px-6 py-8 md:py-10 bg-[#fffaf0] rounded-2xl border border-[#d8a25a]/30 overflow-hidden mb-8 shadow-[0_8px_30px_rgba(146,64,14,0.06)]">
          <h2 className="text-2xl font-extrabold text-[#3d2e22] mb-4">{vedic_philosophy_structure_page.introduc}</h2>
      ...
      {philosophical && <div className="mt-10 relative overflow-hidden rounded-3xl border border-[#d8a25a]/30 p-6 md:p-8 bg-linear-to-br from-[#fffaf3] via-[#fdf1dc] to-[#f8e4c0] shadow-[0_8px_30px_rgba(146,64,14,0.06)]">
          <div className="absolute top-0 left-0 right-0 h-1 bg-linear-to-r from-[#1a6e5c] via-[#d97706] to-[#f59e0b]" />
          <h2 className="text-2xl font-extrabold text-[#3d2e22] mb-4">{vedic_philosophy_structure_page.philosop_explanat}</h2>
          <div className="text-base md:text-lg text-[#5b2d12] leading-relaxed">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
        {links.map(link => <Link key={link.href} href={link.href} className="group block">
            <div className="relative overflow-hidden rounded-2xl border border-[#d8a25a]/50 bg-[#fffaf0] p-6 shadow-[0_8px_30px_rgba(146,64,14,0.08)] transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_20px_60px_rgba(166,61,23,0.18)]">
              ...
              {typeof link.totalSubtopics === 'number' && link.totalSubtopics > 0 && <p className="text-xs text-[#a89278] mt-2">{link.totalSubtopics} {vedic_philosophy_structure_page.subtopic}</p>}
            </div>
          </Link>)}
      </div>
