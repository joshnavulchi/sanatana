...
export default async function Page() {
  ...
  const { data } = await fetchContent(locale, ['vedic-philosophy']);
  if (!data) return notFound();

  const title = (data as any)?.title ?? 'Vedic Philosophy';
  ...
  return (
    <>
      ...
      <PageLayout metaKey="vedic_philosophy_structure" title={title} breadcrumbs={[{ label: 'Home', href: '/' }, { label: title }]} className="layout-md">
          ...
          <section>
            ...
            <ul className="space-y-2">
              {children.map((c: any, i: number) => {
                ...
                const href = `/vedic-philosophy/${String(slug)}`;
                return (
                  <li key={i}>
