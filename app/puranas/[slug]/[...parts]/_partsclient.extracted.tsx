...
export default function PartsClient({
  ...
  const verseNum = slug === 'bhagavata' ? parts.length > 2 ? parseNumericSuffix(parts[2]) : null : parts.length > 1 ? parseNumericSuffix(parts[1]) : null;
  const breadcrumbs: {
    label: string;
    href?: string;
  }[] = [{
    ...
    label: toTitleFromSlug(slug),
    href: `/puranas/${slug}`
  }];
  if (slug === 'bhagavata' && skandaNum !== null) {
    breadcrumbs.push({
      label: `Skanda ${skandaNum}`,
      href: `/puranas/${slug}/skanda-${skandaNum}`
    });
  }
  if (chapterNum !== null) {
    breadcrumbs.push({
      label: `Chapter ${chapterNum}`,
      href: slug === 'bhagavata' ? `/puranas/${slug}/skanda-${skandaNum}/chapter-${chapterNum}` : `/puranas/${slug}/chapter-${chapterNum}`
    });
  }
  if (verseNum !== null) {
    breadcrumbs.push({
  ...
  return <PageLayout metaKey={namespace} title={title} breadcrumbs={breadcrumbs} className="layout-md">
      ...
      {scriptureSections && scriptureSections.length > 0 && <section className="mt-6">
          <h2 className="text-sm font-extrabold uppercase tracking-widest text-[#a89278] mb-3">{page.text}</h2>
          ...
          return <div key={idx} className="rounded-2xl border border-[#edc98f]/50 bg-[#fffaf3] p-5 md:p-6">
      ...
      {verseNum === null && <nav className="mt-8 flex justify-end">
          <Link href={slug === 'bhagavata' ? `/puranas/${slug}/skanda-${skandaNum}/chapter-${chapterNum}/verse-1` : `/puranas/${slug}/chapter-${chapterNum}/verse-1`} className="text-sm font-semibold text-[#7a2e1f] hover:text-[#92400e]"> {page.open_verse} </Link>
        </nav>}
    </PageLayout>;
}