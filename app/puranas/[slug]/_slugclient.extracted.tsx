...
export default function SlugClient({
  ...
  const chapters = Array.isArray(structureNs?.chapters) ? (structureNs.chapters as unknown[]).filter(v => v && typeof v === 'object') as Record<string, unknown>[] : [];
  const skandas = Array.isArray(structureNs?.skandas) ? (structureNs.skandas as unknown[]).filter(v => v && typeof v === 'object') as Record<string, unknown>[] : [];
  if (isLoading) {
    return <PageLayout metaKey={`puranas_${slug}`} title="" breadcrumbs={[{
      label: 'Home',
      href: '/'
    }, {
      label: 'Puranas',
  ...
  return <PageLayout metaKey={`puranas_${slug}`} title={displayTitle} breadcrumbs={[{
    label: 'Home',
    href: '/'
  }, {
    label: 'Puranas',
    ...
    label: displayTitle
      ...
      {scriptureSections.length > 0 && <div className="mt-10">
          ...
          <div className="grid grid-cols-1 gap-6">
            {scriptureSections.map((item, idx) => <SectionCard key={idx} item={item} index={idx} accentFrom="from-[#7c2d12]" accentVia="via-[#c2410c]" accentTo="to-[#fb923c]" />)}
          </div>
        </div>}
      ...
      {isBhagavata && skandas.length > 0 && <section className="mt-10">
          <h2 className="text-2xl font-extrabold text-[#3d2e22] mb-5">{page.skandas}</h2>
          ...
          return <Link key={skandaNumber} href={`/puranas/${slug}/skanda-${skandaNumber}/chapter-1`} className="group block rounded-2xl border border-[#d8a25a]/40 bg-[#fffaf0] p-5 shadow-[0_8px_30px_rgba(146,64,14,0.08)] hover:-translate-y-1 transition-all duration-300">
                  <h3 className="text-lg font-bold text-[#3d2e22] group-hover:text-[#92400e]">{skandaTitle}</h3>
                  <p className="text-sm text-[#6b5d4f] mt-1">{page.open_chapters}</p>
                </Link>;
        })}
          </div>
        </section>}

      {!isBhagavata && chapters.length > 0 && <section className="mt-10">
          <h2 className="text-2xl font-extrabold text-[#3d2e22] mb-5">{page.chapters}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          return <Link key={chapterNumber} href={`/puranas/${slug}/chapter-${chapterNumber}`} className="group block rounded-2xl border border-[#d8a25a]/40 bg-[#fffaf0] p-5 shadow-[0_8px_30px_rgba(146,64,14,0.08)] hover:-translate-y-1 transition-all duration-300">
                  <h3 className="text-lg font-bold text-[#3d2e22] group-hover:text-[#92400e]">{chapterTitle}</h3>
                  <p className="text-sm text-[#6b5d4f] mt-1">{page.open_verses}</p>
                </Link>;
        })}
          </div>
        </section>}
