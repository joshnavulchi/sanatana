    ...
    <PageLayout metaKey={metaKey} title={title} description={description} breadcrumbs={breadcrumbs} className="layout-md bg-gradient-to-br from-[#fffbe6] via-[#ffe7ba] to-[#ffd591] min-h-screen animate-fade-in">
      ...
      {typeof data?.introduction === 'string' && (
        <section className="rounded-2xl bg-gradient-to-br from-[#fffbe6]/80 via-[#ffe7ba]/80 to-[#ffd591]/80 shadow-xl border border-[#ffe7ba]/40 p-6 mb-8 animate-fade-in-up">
          <p className="text-lg sm:text-base leading-relaxed mb-4 font-normal text-[#ad6800]">{data.introduction}</p>
        </section>
      )}
      {/* Sections array */}
          ...
          {data.section.map((item: any, idx: number) => {
            ...
            return (
              <section key={idx}>
                ...
                {secContent ? (
                  <p className="text-lg sm:text-base leading-relaxed mb-4 font-normal">{secContent}</p>
                ) : null}
              </section>
            );
          })}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-lg sm:text-base leading-relaxed font-normal">
            {data.chapters.map((c: any, i: number) => (
              <a key={`chapter-${i}`} href={c?.path || '#'} className="block rounded-lg p-4 bg-white hover:shadow-md">
                <div className="text-lg sm:text-base leading-relaxed font-normal">{String(c?.title ?? `Chapter ${c?.chapter ?? i + 1}`)}</div>
            ...
            {data.mandalas.map((m: any, i: number) => (
              <a key={`mandala-${i}`} href={m?.path || '#'} className="block rounded-lg p-4 bg-white hover:shadow-md">
                <div className="flex items-baseline justify-between text-lg sm:text-base leading-relaxed font-normal">
                  <div className="text-amber-800 text-lg sm:text-base leading-relaxed font-normal">Mandala {String(m?.mandala ?? m?.number ?? i + 1)}</div>
