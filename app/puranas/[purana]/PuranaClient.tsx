    ...
    <PageLayout metaKey={metaKey} title={title} description={description} breadcrumbs={breadcrumbs} className="layout-md bg-gradient-to-br from-[#fffbe6] via-[#ffe7ba] to-[#ffd591] min-h-screen animate-fade-in">
      ...
      {typeof data?.introduction === 'string' && (
        <section className="rounded-2xl bg-gradient-to-br from-[#fffbe6]/80 via-[#ffe7ba]/80 to-[#ffd591]/80 shadow-xl border border-[#ffe7ba]/40 p-6 mb-8 animate-fade-in-up">
          <p className="text-md sm:text-base leading-relaxed mb-4 font-normal text-[#ad6800]">{data.introduction}</p>
        </section>
      )}
      {/* Sections array */}
        ...
        <>
          {data.section.map((item: any, idx: number) => {
            ...
            return (
              <section key={idx}>
                {secTitle ? (
                  <h3 className="text-red-800 text-2xl font-semibold leading-snug mb-3">{secTitle}</h3>
                ) : null}
                {secContent ? (
                  <p className="text-md sm:text-base leading-relaxed mb-4 font-normal">{secContent}</p>
                ) : null}
              </section>
            );
          })}
          ...
          {data.scripture_text.map((item: any, i: number) => {
            ...
            return (
              <section key={`scripture-${i}`}>
                ...
                {sContent ? (
                  <p className="text-md sm:text-base leading-relaxed mb-4 font-normal">{sContent}</p>
                ) : null}
              </section>
            );
          })}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-md sm:text-base leading-relaxed font-normal">
            {data.chapters.map((c: any, i: number) => (
              <a key={`chapter-${i}`} href={c?.path || '#'} className="block rounded-lg p-4 bg-white hover:shadow-md">
                <div className="text-md sm:text-base leading-relaxed font-normal">{String(c?.title ?? `Chapter ${c?.chapter ?? i + 1}`)}</div>
        ...
        <section className="mt-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-md sm:text-base leading-relaxed font-normal">
            {data.major_rigvedic_deities.map((d: any, i: number) => (
              <div key={`deity-${i}`} className="rounded-lg p-4 bg-white text-md sm:text-base leading-relaxed font-normal">
            ...
            {data.mandalas.map((m: any, i: number) => (
              <a key={`mandala-${i}`} href={m?.path || '#'} className="block rounded-lg p-4 bg-white hover:shadow-md">
                <div className="flex items-baseline justify-between text-md sm:text-base leading-relaxed font-normal">
                  <div className="text-amber-800 text-md sm:text-base leading-relaxed font-normal">Mandala {String(m?.mandala ?? m?.number ?? i + 1)}</div>
        ...
        <section className="mt-6">
          ...
          <div className="flex flex-wrap gap-2 text-md sm:text-base leading-relaxed font-normal">
            {data.related_concepts.map((c: any, i: number) => (
              <span key={`concept-${i}`} className="inline-flex items-center px-3 py-1 rounded-full bg-amber-50 text-amber-800 text-md sm:text-base leading-relaxed font-normal">{String(c)}</span>
            ))}
          </div>
        </section>
      )}
        ...
        if (val) {
          return (
            <section>
              ...
              <p className="text-md sm:text-base leading-relaxed mb-4 font-normal">{val}</p>
            </section>
          );
        }
        return null;
