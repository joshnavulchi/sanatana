...
const contact_page = {
  ...
  sanatana: "@SanatanaDharmaMin",
  support_sanatana: "support@sanatanadharmam.in",
  www_sanatana: "www.sanatanadharmam.in"
};
...
function RenderNode({
  ...
  if (Array.isArray(node)) {
    const items = node.filter(it => !(typeof it === 'string' && /^(true|false)$/i.test(String(it).trim()))).map((it, i) => <li key={i}><RenderNode node={it} /></li>);
...
export default function ContactPage() {
  ...
  if (isLoading && !page.title) {
    return <PageLayout metaKey="contact" title="" breadcrumbs={[{
      labelKey: 'Home',
      href: '/'
    }, {
      label: 'Contact'
  ...
  return <PageLayout metaKey="contact" title={page.title} breadcrumbs={[{
    labelKey: 'Home',
    href: '/'
  }, {
    label: page.title || 'Contact'
      ...
      <div className="flex flex-col md:flex-row gap-8 md:gap-0 bg-white/80 rounded-3xl shadow-2xl overflow-hidden border-2 border-amber-100">
        ...
        <div className="relative md:w-2/5 flex flex-col justify-between bg-gradient-to-br from-amber-500 via-orange-400 to-amber-600 text-white p-8 md:p-10 gap-8">
          <div>
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h2 className="text-2xl tracking-tight">{contact_page.contact_us}</h2>
            ...
            <div className="space-y-4 mt-8">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-xl md:text-lg">{contact_page.support_sanatana}</span>
              </div>
              <div className="flex items-center gap-3">
                <span className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-white/10">
                ...
                <span className="text-xl md:text-lg">{contact_page.sanatana}</span>
              </div>
              <div className="flex items-center gap-3">
                <span className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-white/10">
                ...
                <span className="text-xl md:text-lg">{contact_page.www_sanatana}</span>
              </div>
            </div>
          </div>
          {/* Social links */}
        ...
        <div className="md:w-3/5 flex flex-col justify-center bg-white">
          <ContactForm />
        </div>
      </div>
      {/* FAQ Section */}
      {page.faq && <div className="mt-12">
