...
export default function Page({ searchParams }: any) {
  ...
  const localeNamespace = getLocaleNamespaceObject(locale, 'kidszone_illustratedstories') as any;
  const nsObj: any = localeNamespace?.kidszone_illustratedstories || localeNamespace || {};
    ...
    <>
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
          {stories.map((s: any) => (
            <Link key={s.id} href={`/kidszone/illustratedstories/${s.id}`} className="group block rounded-xl overflow-hidden shadow-lg border border-pink-200 bg-linear-to-br from-pink-50 to-yellow-50 hover:shadow-2xl transition-transform hover:-translate-y-1">
