  ...
  if (!started) {
    return (
      <div>
        ...
        <h3 className="text-xl md:text-lg md:text-md md:text-2xl">{(ns?.readyDescription || 'This quiz has {count} questions, time {time}').replace('{count}', String(qList.length)).replace('{time}', fmtTime(timeLeft))}</h3>
        <div className="flex items-center justify-between mt-6">
          <button className="cursor-pointer group md:inline-flex px-4 py-2 bg-white/10 backdrop-blur-md
                    ...
                    text-amber-500  text-lg md:text-md rounded-full shadow-lg hover:shadow-xl
    ...
    return (
      <div>
        ...
        <div className="flex items-center justify-between my-6">
          <div className="text-xl md:text-lg md:text-md">{ns?.yourScore || 'Your score:'} <strong>{score}</strong> / {qList.length}</div>
          <div className="text-xl md:text-lg md:text-md">{ns?.timeTaken || 'Time taken:'} {fmtTime(10 * 60 - timeLeft)}</div>
        </div>
        <div>
          {qList.map((q, idx) => (
            <div key={q.id}>
              <div className="text-xl md:text-lg md:text-md font-semibold">{idx + 1}. {q.question}</div>
              <div className="text-lg md:text-md ml-4 my-2">
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="text-xl md:text-lg md:text-md md:text-2xl font-semibold">{(ns?.questionCounter || 'Question {current} / {total}').replace('{current}', String(current + 1)).replace('{total}', String(qList.length))}</div>
        <div className=''>{ns?.timeLeftLabel || 'Time left:'} {fmtTime(timeLeft)}</div>
      </div>

      <div className="flex flex-col justify-start items-start gap-6">
        <div className="text-xl md:text-lg md:text-md md:text-2xl font-bold">{q.question}</div>
        <div className="text-xl md:text-lg md:text-md md:text-2xl font-semibold mt-4 flex flex-col gap-3">
          {(['A', 'B', 'C', 'D'] as (keyof Options)[]).map((k) => {
            const isSelected = answers[q.id] === k;
            return (
              <button
                className={
                  `cursor-pointer group md:inline-flex px-4 py-2 border-2 text-lg md:text-md rounded-full shadow-lg transition-all duration-300 transform no-underline ` +
                  (isSelected
