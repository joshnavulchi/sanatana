...
export default function JyotirlingsClient() {
  ...
  if (isLoading && !data.title) {
    return (
      <PageLayout metaKey="jyotirlings" title="" breadcrumbs={[{ labelKey: 'Home', href: '/' }, { label: 'Jyotirlings' }]} className="layout-sm">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="w-full lg:w-3/4">
            ...
            {data.items.map((section: any, index: number) => {
              ...
              return (
                <div key={section.id || index} className="relative bg-white border-2 border-yellow-100 rounded-2xl p-6 md:p-8 mt-12 transform-gpu transition-transform duration-300 hover:-translate-y-1 hover:scale-104 ring-1 ring-yellow-100/30 bg-white/80 backdrop-blur-sm group overflow-hidden">
                  ...
                  <div className="relative z-10 space-y-4">
                    <div className="flex items-start gap-4">
                      <div className="flex-shrink-0 w-12 h-12 bg-gradient-to-br from-yellow-100 to-amber-100 rounded-xl flex items-center justify-center text-2xl ring-1 ring-yellow-50/40 group-hover:scale-105 group-hover:rotate-3 transition-transform duration-300">{icon}</div>
                    ...
                    {section?.unique_features && <p className="text-base md:text-lg leading-relaxed pl-16">{Array.isArray(section.unique_features) ? section.unique_features.join(', ') : section.unique_features}</p>}
                    {section?.dynasties && <p className="text-base md:text-lg leading-relaxed pl-16">{Array.isArray(section.dynasties) ? section.dynasties.join(', ') : section.dynasties}</p>}
                  </div>
                </div>
              );
            })}
