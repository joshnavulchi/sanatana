        ...
        return (
          <div key={section.id || index} className="relative bg-white border-l-4 border-amber-300 rounded-2xl p-6 md:p-8 mt-12 shadow-xl hover:shadow-2xl transition-all duration-500 group overflow-hidden">
            ...
            <div className="relative z-10 space-y-4">
              ...
              {section?.timeline?.early_mentions && <p className="text-md leading-relaxed ">{section.timeline.early_mentions}</p>}
              {section?.timeline?.ancient_mentions && <p className="text-md leading-relaxed ">{section.timeline.ancient_mentions}</p>}
              {section?.unique_features && <p className="text-md leading-relaxed ">{Array.isArray(section.unique_features) ? section.unique_features.join(', ') : section.unique_features}</p>}
              {section?.dynasties && <p className="text-md leading-relaxed ">{Array.isArray(section.dynasties) ? section.dynasties.join(', ') : section.dynasties}</p>}
            </div>
          </div>
        );
      })}
