  ...
  return (
    <div className={`relative overflow-hidden rounded-3xl border border-amber-200/30 p-6 md:p-8 ${shells[index % 3]} shadow-[0_8px_30px_rgba(146,64,14,0.06)]`}>
      ...
      <div className="flex items-center gap-3 mb-5 pl-2">
        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl text-xs font-extrabold text-white shadow-[0_4px_20px_rgba(122,46,31,0.25)]">
          {index + 1}
        </span>
...
export default function BhagavadGitaClient() {
  ...
  return (
    <PageLayout metaKey="itihasa_bhagavadgita" title={title} breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Itihasa', href: '/itihasa' }, { label: title }]} className="layout-md">
      {description && (
        <div className="relative px-4 md:px-6 py-4 md:py-12 rounded-2xl border-amber-200/30 bg-amber-50 overflow-hidden mb-6">
          <p className="text-base body-text">{description}</p>
        </div>
      )}

      {introduction && (
        <div className="relative px-4 md:px-6 py-4 md:py-10 rounded-2xl border-amber-200/30 bg-amber-50 overflow-hidden mb-6 shadow-[0_8px_30px_rgba(146,64,14,0.06)]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {chapters.map((link, idx) => (
              <Link key={link.href} href={link.href} className="group block">
                <div className="relative overflow-hidden rounded-2xl border-amber-200/50 bg-amber-50 p-4 shadow-[0_8px_30px_rgba(146,64,14,0.08)] transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_20px_60px_rgba(166,61,23,0.18)]">
                  <div className="absolute top-0 left-0 right-0 h-1" />
                  <div className="flex items-center gap-3 mt-2">
                    <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-primary-700/15 text-sm font-semibold text-primary-700">{idx + 1}</span>
                    <h3 className="text-base font-semibold text-gray-900 group-hover:text-primary-700 transition-colors">{link.label}</h3>
                  </div>
                </div>
              </Link>
            ))}
