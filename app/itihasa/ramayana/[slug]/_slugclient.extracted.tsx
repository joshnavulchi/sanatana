...
export default function SlugClient({
  ...
  const ns = useLocaleSection(`ramayana_${slug}`);
  ...
  if (isLoading) {
    return <PageLayout metaKey={`ramayana_${slug}`} title="" breadcrumbs={[{
      label: 'Home',
      href: '/'
    }, {
      label: 'Itihasa',
  ...
  return <PageLayout metaKey={`ramayana_${slug}`} title={displayTitle} breadcrumbs={[{
    label: 'Home',
    href: '/'
  }, {
    label: 'Itihasa',
    ...
    label: displayTitle
      ...
      {scriptureSections.length > 0 && <div className="mt-10">
          ...
          <div className="grid grid-cols-1 gap-6">
            {scriptureSections.map((item, idx) => <SectionCard key={idx} item={item} index={idx} accentFrom="from-[#7a2e1f]" accentVia="via-[#c2410c]" accentTo="to-[#f59e0b]" />)}
          </div>
        </div>}
      ...
      <div className="mt-8 flex justify-end">
        <Link href={`/itihasa/ramayana/${slug}/sarga-1`} className="text-sm font-semibold text-[#7a2e1f] hover:text-[#92400e]"> {page.open_sarga} </Link>
      </div>
    </PageLayout>;
}
