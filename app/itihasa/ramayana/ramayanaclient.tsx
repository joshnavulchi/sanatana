  ...
  return (
    <>
      {text.split('\n\n').map((p, i) => (
        <p
          key={i}
          className={`mb-4 last:mb-0 ${className} transition-all duration-500 ease-in-out bg-linear-to-r from-amber-50/80 to-amber-100/60 rounded-xl px-3 py-2 shadow-sm hover:shadow-lg`}
        >
          {p}
        </p>
      ))}
...
function SectionCard({ item, index }: { item: Record<string, unknown>; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute left-0 top-1 bottom-0 w-1 bg-linear-to-b from-amber-200/60 to-amber-100/0 animate-pulse" />
...
export default function RamayanaClient() {
  ...
  const ns = useLocaleSection('itihasa/ramayana/index');
  ...
  if (isLoading && !ns?.title && !ramayanaEntry) {
    return (
      <PageLayout metaKey="itihasa/ramayana/index" title="" breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Itihasa', href: '/itihasa' }, { label: 'Ramayana' }]} className="layout-md">
  ...
  return (
    <PageLayout metaKey="itihasa/ramayana/index" title={title} breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Itihasa', href: '/itihasa' }, { label: title }]} className="layout-md">
      {description && (
      ...
      {philosophical && (
        <div className="mt-10 relative overflow-hidden rounded-3xl border-amber-200/30 p-6 md:p-8 bg-linear-to-br from-yellow-50 via-amber-100 to-amber-200 shadow-xl animate-fadeIn">
          <div className="absolute top-0 left-0 right-0 h-1 bg-linear-to-r from-amber-200/60 to-amber-100/0 animate-pulse" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {kandas.map((link, idx) => (
              <Link key={link.href} href={link.href} className="group block">
                <div className="relative overflow-hidden rounded-2xl border-amber-200/50 bg-linear-to-br from-amber-50 via-amber-100 to-yellow-50 p-6 shadow-lg transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl hover:bg-amber-100/80 animate-fadeIn">
                  <div className="absolute top-0 left-0 right-0 h-1 bg-linear-to-r from-amber-200/60 to-amber-100/0 animate-pulse" />
