...
const scriptures_ramayana_page = {
...
export default function RamayanaClient() {
  ...
  const ramayanaEntry = Array.isArray(itihasaNav) ? itihasaNav.find((item: Record<string, unknown>) => item?.name === 'Ramayana') : null;
  const kandas: NavLink[] = ramayanaEntry?.kandas ? Object.entries(ramayanaEntry.kandas as Record<string, string>).map(([key, val]) => ({
  ...
  if (isLoading && !ns?.title && !ramayanaEntry) {
    return <PageLayout metaKey="scriptures_ramayana" title="" breadcrumbs={[{
      label: 'Home',
      href: '/'
    }, {
      label: 'Itihasa',
  ...
  return <PageLayout metaKey="scriptures_ramayana" title={title} breadcrumbs={[{
    label: 'Home',
    href: '/'
  }, {
    label: 'Itihasa',
    ...
    label: title
      ...
      {introduction && <div className="relative px-4 md:px-6 py-8 md:py-10 bg-[#fffaf0] rounded-2xl border border-[#d8a25a]/30 overflow-hidden mb-8 shadow-[0_8px_30px_rgba(146,64,14,0.06)]">
          <h2 className="text-2xl font-extrabold text-[#3d2e22] mb-4">{scriptures_ramayana_page.introduc}</h2>
      ...
      {scriptureSections.length > 0 && <div className="mt-8">
          <h2 className="text-2xl font-extrabold text-[#3d2e22] mb-6">{scriptures_ramayana_page.overview}</h2>
      ...
      {philosophical && <div className="mt-10 relative overflow-hidden rounded-3xl border border-[#d8a25a]/30 p-6 md:p-8 bg-linear-to-br from-[#fffaf3] via-[#fdf1dc] to-[#f8e4c0] shadow-[0_8px_30px_rgba(146,64,14,0.06)]">
          ...
          <h2 className="text-2xl font-extrabold text-[#3d2e22] mb-4">{scriptures_ramayana_page.philosop_explanat}</h2>
          <div className="text-base md:text-lg text-[#5b2d12] leading-relaxed">
            <Paragraphs text={philosophical} />
          </div>
        </div>}

      {kandas.length > 0 && <div className="mt-8">
          <h2 className="text-2xl font-bold text-[#3d2e22] mb-6">{scriptures_ramayana_page.kandas}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {kandas.map((link, idx) => <Link key={link.href} href={link.href} className="group block">
                <div className="relative overflow-hidden rounded-2xl border border-[#d8a25a]/50 bg-[#fffaf0] p-6 shadow-[0_8px_30px_rgba(146,64,14,0.08)] transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_20px_60px_rgba(166,61,23,0.18)]">
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    <h3 className="text-lg font-bold text-[#3d2e22] group-hover:text-[#7a2e1f] transition-colors">{link.label}</h3>
                  </div>
                </div>
              </Link>)}
          </div>
