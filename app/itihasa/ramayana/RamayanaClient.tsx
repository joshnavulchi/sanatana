  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-5 pl-2">
        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl text-sm font-semibold text-amber-50 bg-amber-500/80 shadow-lg">
          {index + 1}
        </span>
        <h3 className="text-xl md:text-2xl font-semibold text-amber-900 drop-shadow">{section}</h3>
      </div>
      <div className="text-md sm:text-base text-amber-800 leading-relaxed pl-2">
    ...
    <PageLayout metaKey={META_KEY} title={title} breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Itihasa', href: '/itihasa' }, { label: title }]} className="layout-md">
      {description && (
        <div className="relative px-4 md:px-6 py-8 md:py-12 bg-gradient-to-br from-amber-50 via-amber-100 to-yellow-50 rounded-2xl border-amber-200/30 overflow-hidden mb-8 shadow-lg animate-fadeIn">
          <p className="text-md sm:text-base body-text text-amber-900 drop-shadow">{description}</p>
        </div>
      )}
              ...
              <Link key={link.href} href={link.href} className="group block">
                <div className="relative overflow-hidden rounded-2xl border-amber-200/50 bg-gradient-to-br from-amber-50 via-amber-100 to-yellow-50 p-6 shadow-lg transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl hover:bg-amber-100/80 animate-fadeIn">
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-amber-700/15 text-md sm:text-base font-semibold text-amber-700 shadow-md">{idx + 1}</span>
                    <h6 className="text-md sm:text-base font-semibold text-amber-900 group-hover:text-amber-700 transition-colors">{link.label}</h6>
                  </div>
                </div>
              </Link>
            ))}
