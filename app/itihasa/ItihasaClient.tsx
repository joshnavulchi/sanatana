...
export default function ItihasaClient() {
  ...
  if (isLoading && !section) {
    return (
      <PageLayout metaKey="itihasa" title="" breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Itihasa' }]} className="layout-md">
        <div className="flex items-center justify-center py-6 text-base sm:text-lg leading-relaxed font-normal"><Loader /></div>
      </PageLayout>
    );
  }
  ...
  return (
    <PageLayout metaKey="itihasa" title={title} breadcrumbs={[{ label: 'Home', href: '/' }, { label: title }]} className="layout-md">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 text-base sm:text-lg leading-relaxed font-normal bg-gradient-to-br from-orange-50 via-amber-100 to-yellow-50 rounded-3xl border border-orange-200/30 shadow-xl p-8 md:p-12 animate-fadeIn">
        {links.map((link) => (
          <Link key={link.href} href={link.href} className="group block">
            <div className="relative overflow-hidden rounded-2xl border-orange-200/50 bg-gradient-to-r from-orange-100/80 to-amber-50/60 p-6 shadow-lg transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl hover:bg-orange-100/80 animate-fadeIn">
              ...
              {link.description && <p className="meta-text line-clamp-2 text-base sm:text-lg leading-relaxed mb-4 font-normal text-orange-800/90 bg-gradient-to-r from-orange-50/80 to-amber-100/60 rounded-lg px-3 py-2 shadow-sm">{link.description}</p>}
            </div>
          </Link>
        ))}
      </div>
