  ...
  return (
    <>
      {text.split('\n\n').map((paragraph, index) => (
        <p
          key={index}
          className="last:mb-0 text-md leading-relaxed mb-4 font-normal bg-gradient-to-r from-orange-50/80 to-amber-100/60 rounded-xl px-3 py-2 shadow-sm hover:shadow-lg transition-all duration-500"
        >
          {paragraph}
        </p>
      ))}
  ...
  if (isLoading && !title) {
    return (
      <PageLayout metaKey={namespace} title="" breadcrumbs={breadcrumbs} className="layout-md">
        <div className="flex items-center justify-center py-12 text-md leading-relaxed font-normal">
          <Loader />
        </div>
      </PageLayout>
    );
  ...
  return (
    <PageLayout metaKey={namespace} title={title} breadcrumbs={breadcrumbs} className="layout-md">
      <section className="rounded-3xl border-orange-200/30 bg-gradient-to-br from-orange-50 via-amber-100 to-yellow-50 p-8 md:p-12 shadow-xl animate-fadeIn">
        ...
        {description && <p className="body-text text-md leading-relaxed mb-4 font-normal text-orange-800/90 bg-gradient-to-r from-orange-100/60 to-amber-50/40 rounded-xl px-4 py-2 shadow-sm">{description}</p>}
      </section>
      ...
      {introduction && (
        <section className="mt-6 rounded-2xl border-orange-200/30 bg-gradient-to-r from-orange-50/80 to-amber-100/60 p-6 shadow-lg animate-fadeIn">
          ...
          <div className="body-text text-md leading-relaxed font-normal">
      ...
      {nextHref && (
        <nav className="mt-8 flex justify-end">
          <Link href={nextHref} className="text-md font-semibold text-orange-600 hover:text-orange-700 transition-colors">
            {nextLabel || 'Open next'} →
          </Link>
        </nav>
      )}
