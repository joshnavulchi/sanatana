...
type Props = {
  initialData?: ParvaData | null;
  initialLocale?: string;
  parva: string[];
};

export default function ParvaClient({ initialData, initialLocale, parva }: Props) {
  const [data, setData] = useState<ParvaData | null>(initialData || null);
  ...
  function Paragraph({ children }: any) {
    return <p className="text-md leading-relaxed mb-4">{children}</p>;
  }
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const res = await fetchContentByRoute(locale, ['itihasa', 'mahabharata', ...(parva || [])]);
        ...
        if (parva && parva.length > 0 && typeof parsed === 'object') {
          const key = parva[0];
  ...
  }, [initialData, initialLocale, ctxLocale, locale, parva]);

  const title = String(data?.meta?.title ?? data?.title ?? parva.join(' / ') ?? 'Mahabharata');
  ...
  if (parva && parva.length > 0) breadcrumbs.push({ label: parva[0], href: `/itihasa/mahabharata/${parva[0]}` });

  const metaKey = (data?.meta?.key && String(data.meta.key)) || `itihasa/mahabharata/${parva.join('/')}/index`;
