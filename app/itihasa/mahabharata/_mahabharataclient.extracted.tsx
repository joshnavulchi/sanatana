    ...
    <>
      ...
      <PageLayout metaKey="itihasa_mahabharata" title={title} breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Itihasa', href: '/itihasa' }, { label: title }]} className="layout-md">
        ...
        {scriptureSections.length > 0 && (
          <div className="mt-8">
            <h2 className="text-2xl font-extrabold text-[#3d2e22] mb-4">{itihasa_mahabharata_page.overview}</h2>
        ...
        <div className="mt-8">
          <h2 className="text-2xl font-bold text-[#3d2e22] mb-4">{itihasa_mahabharata_page.parvas}</h2>
              ...
              <Link key={parva} href={`/itihasa/mahabharata/${parva}`} className="group block">
                <div className="relative overflow-hidden rounded-2xl border border-[#d8a25a]/50 bg-[#fffaf0] p-4 shadow-[0_8px_30px_rgba(146,64,14,0.08)] transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_20px_60px_rgba(166,61,23,0.18)]">
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    <h3 className="text-base font-bold text-[#3d2e22] group-hover:text-[#7a2e1f] transition-colors">{toTitleFromSlug(parva)}</h3>
                  </div>
                </div>
              </Link>
            ))}
