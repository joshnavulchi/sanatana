...
const itihasa_mahabharata_page = {
...
export default function MahabharataClient() {
  ...
  if (isLoading && !ns?.title) {
    return <PageLayout metaKey="scriptures_mahabharata" title="" breadcrumbs={[{
      label: 'Home',
      href: '/'
    }, {
      label: 'Itihasa',
  ...
  return <>
      ...
      <PageLayout metaKey="itihasa_mahabharata" title={title} breadcrumbs={[{
      label: 'Home',
      href: '/'
    }, {
      label: 'Itihasa',
      ...
      label: title
        ...
        {introduction && <div className="relative px-4 md:px-6 py-8 md:py-10 bg-[#fffaf0] rounded-2xl border border-[#d8a25a]/30 overflow-hidden mb-8 shadow-[0_8px_30px_rgba(146,64,14,0.06)]">
            <h2 className="text-2xl font-extrabold text-[#3d2e22] mb-4">{itihasa_mahabharata_page.introduc}</h2>
        ...
        {scriptureSections.length > 0 && <div className="mt-8">
            <h2 className="text-2xl font-extrabold text-[#3d2e22] mb-6">{itihasa_mahabharata_page.overview}</h2>
        ...
        {philosophical && <div className="mt-10 relative overflow-hidden rounded-3xl border border-[#d8a25a]/30 p-6 md:p-8 bg-linear-to-br from-[#fffaf3] via-[#fdf1dc] to-[#f8e4c0] shadow-[0_8px_30px_rgba(146,64,14,0.06)]">
            ...
            <h2 className="text-2xl font-extrabold text-[#3d2e22] mb-4">{itihasa_mahabharata_page.philosop_explanat}</h2>
            <div className="text-base md:text-lg text-[#5b2d12] leading-relaxed">
        ...
        <div className="mt-8">
          <h2 className="text-2xl font-bold text-[#3d2e22] mb-6">{itihasa_mahabharata_page.parvas}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {MAHABHARATA_PARVAS.map((parva, idx) => <Link key={parva} href={`/itihasa/mahabharata/${parva}`} className="group block">
