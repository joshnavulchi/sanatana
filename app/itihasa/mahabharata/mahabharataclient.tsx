...
export default function MahabharataClient() {
  ...
  return (
    <PageLayout metaKey="itihasa_mahabharata" title={title} breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Itihasa', href: '/itihasa' }, { label: title }]} className="layout-md">
      {description && (
        <div className="relative px-4 md:px-6 py-8 md:py-12 rounded-2xl border-amber-200/30 bg-amber-50 overflow-hidden mb-8">
          <p className="text-base body-text">{description}</p>
        </div>
      )}

      {introduction && (
        <div className="relative px-4 md:px-6 py-8 md:py-10 rounded-2xl border-amber-200/30 bg-amber-50 overflow-hidden mb-8 shadow-[0_8px_30px_rgba(146,64,14,0.06)]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {MAHABHARATA_PARVAS.map((parva, idx) => (
            <Link key={parva} href={`/itihasa/mahabharata/${parva}`} className="group block">
              <div className="relative overflow-hidden rounded-2xl border-amber-200/50 bg-amber-50 p-5 shadow-[0_8px_30px_rgba(146,64,14,0.08)] transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_20px_60px_rgba(166,61,23,0.18)]">
                <div className="absolute top-0 left-0 right-0 h-1" />
