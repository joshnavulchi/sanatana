...
import { toTitleFromSlug } from '@lib/siteUtils';
  ...
  return (
    <>
      {text.split('\n\n').map((p, i) => (
        <p
          key={i}
          className={`mb-4 last:mb-0 ${className} transition-all duration-500 ease-in-out bg-linear-to-r from-indigo-50/80 to-blue-100/60 rounded-xl px-3 py-2 shadow-sm hover:shadow-lg`}
        >
          {p}
        </p>
      ))}
...
function SectionCard({ item, index }: { item: Record<string, unknown>; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute left-0 top-1 bottom-0 w-1 bg-linear-to-b from-blue-200/60 to-indigo-100/0 animate-pulse" />
...
export default function MahabharataClient() {
  ...
  const ns = useLocaleSection('itihasa/mahabharata/index');
  ...
  // Load parvas from the locale structure JSON
  const parvasFromLocale = Array.isArray(ns?.parvas)
    ? (ns.parvas as Array<{ slug: string; title: string }>)
    : [];

  if (isLoading && !ns?.title) {
    return (
      <PageLayout metaKey="itihasa/mahabharata/index" title="" breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Itihasa', href: '/itihasa' }, { label: 'Mahabharata' }]} className="layout-md">
  ...
  return (
    <PageLayout metaKey="itihasa/mahabharata/index" title={title} breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Itihasa', href: '/itihasa' }, { label: title }]} className="layout-md">
      {description && (
      ...
      {philosophical && (
        <div className="mt-10 relative overflow-hidden rounded-3xl border-blue-200/30 p-6 md:p-8 bg-linear-to-br from-blue-50 via-indigo-100 to-blue-200 shadow-xl animate-fadeIn">
          <div className="absolute top-0 left-0 right-0 h-1 bg-linear-to-r from-blue-200/60 to-indigo-100/0 animate-pulse" />
      ...
      <div className="mt-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {parvasFromLocale.map((parva, idx) => (
            <Link key={parva.slug} href={`/itihasa/mahabharata/${parva.slug}`} className="group block">
              <div className="relative overflow-hidden rounded-2xl border-blue-200/50 bg-linear-to-r from-blue-100/80 to-indigo-50/60 p-5 shadow-lg transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl hover:bg-blue-100/80 animate-fadeIn">
                <div className="absolute top-0 left-0 right-0 h-1 bg-linear-to-r from-blue-200/60 to-indigo-100/0 animate-pulse" />
                <div className="flex items-center gap-3 mt-2">
                  ...
                  <h3 className="text-base font-semibold text-blue-900 group-hover:text-blue-700 transition-colors">
                    {parva.title || toTitleFromSlug(parva.slug)}
                  </h3>
                </div>
              </div>
            </Link>
