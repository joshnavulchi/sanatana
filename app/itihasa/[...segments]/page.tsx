...
export default async function Page({ params }: Props) {
  ...
  const segments = ['itihasa', ...rawSegments];
  ...
  const crumbs = [{ label: 'Home', href: '/' }, { label: 'Itihasa', href: '/itihasa' }];
  let accum: string[] = [];
  for (const s of rawSegments) {
    accum.push(s);
    crumbs.push({ label: s, href: `/itihasa/${accum.join('/')}` });
  }

  return (
    <>
      <StructuredData metaKey="itihasa" />
      <PageLayout metaKey="itihasa" title={title} breadcrumbs={crumbs} className="layout-md">
          ...
          <section>
            ...
            <ul className="space-y-2">
              {children.map((c: any, i: number) => {
                ...
                const href = `/itihasa/${[...rawSegments, String(slug)].join('/')}`;
                return (
                  <li key={i}>
