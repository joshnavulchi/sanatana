          ...
          <div className="relative md:-mx-8 px-6 md:px-8 py-12 bg-gradient-to-br from-blue-50 via-indigo-50 to-blue-50 rounded-2xl overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              {pageContent.meta?.description && (
                <p className="text-lg md:text-md md:text-xl md:text-lg md:text-md  leading-relaxed mb-4">
                  {pageContent.meta.description}
                </p>
              )}
              {pageContent.intro && (
                <p className="text-xl md:text-lg md:text-md md:text-lg md:text-md text-gray-600 leading-relaxed">{pageContent.intro}</p>
              )}
              {pageContent.description && (
                <p className="text-xl md:text-lg md:text-md md:text-lg md:text-md text-gray-600 leading-relaxed">{pageContent.description}</p>
              )}
            </div>
          </div>
                ...
                <div className="flex flex-wrap gap-6">
                  {pageContent.whyBordersBecameStable.factors.map((factor: any, i: number) => (
                    ...
                    ">
                      ...
                      <p className="text-xl md:text-lg md:text-md  leading-relaxed">{factor.explanation}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="flex gap-4 flex-wrap">
                {pageContent.legend.changeTypes.map((ct: any) => (
                  ...
                  ">
                    ...
                    />
                    <span className="text-xl md:text-lg md:text-md font-medium ">{ct.label}</span>
                  </div>
                ))}
              </div>
            </section>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xl md:text-lg md:text-md text-gray-600">{pageContent.decades.length} periods of transformation</p>
                  </div>
                </div>
              </div>
                  ...
                  <div key={d.key ?? d.decade ?? d.label ?? idx} className="relative">
                    ...
                    <div className="md:ml-20 bg-white  border-2 border-blue-100  hover:border-blue-300  rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 group">
                      <div className="flex items-start justify-between gap-4 mb-4">
                        <h4 className="text-xl md:text-lg md:text-md font-bold text-gray-900  group-hover:text-blue-600  transition-colors">
                          {d.decade ?? d.label ?? d.title ?? `Decade ${idx + 1}`}
                        </h4>
                        <span className="flex-shrink-0 px-3 py-1 bg-gradient-to-r from-blue-100 to-indigo-100   text-blue-700  text-xl md:text-lg md:text-md font-semibold rounded-full">
                          #{idx + 1}
                        </span>
                      </div>
                      ...
                      {d.whatChanged && Array.isArray(d.whatChanged) && d.whatChanged.length > 0 && (
                        <div className="mb-4">
                          <h5 className="text-xl md:text-lg md:text-md font-bold text-gray-900  mb-2 flex items-center gap-2">
                            <span>🔄</span>
                            What Changed:
                          </h5>
                          <ul className="space-y-2">
                            {d.whatChanged.map((w: string, i: number) => (
                              <li key={i} className="flex items-start gap-2 text-xl md:text-lg md:text-md  ">
                      ...
                      {d.svgOverlay && (
                        <div className="pt-4 border-t border-gray-200 ">
                          <label className="inline-flex items-center gap-3 cursor-pointer group/checkbox">
                            ...
                            />
                            <span className="text-xl md:text-lg md:text-md font-medium   group-hover/checkbox:text-blue-600  transition-colors">
