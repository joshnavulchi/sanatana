...
const historical_timeline_meta_page = {
  astronom: "Astronomical",
  astronom_system: "Astronomy System:",
  century_rules: "Century Rules:",
  common_mistakes: "Common Mistakes:",
  counting_directio: "Counting Direction:",
  duration: "Duration:",
  dynasty: "Dynasty:",
  ...
  exists: "Exists:",
  historic: "Historical",
  historic_rulers: "Historical Rulers by Region",
  history_system: "History System:",
  implicat: "Implications:",
  mapping: "Mapping:",
  new_bce: "New (BCE/CE)",
  note: "Note:",
  notes: "Notes:",
  ...
  reign: "Reign:",
  rules: "Rules:",
  sequence_example: "Sequence Example:",
  time_period: "Time Period:",
  transiti: "Transition:",
  uses_year: "Uses Year Zero:",
...
const Region = ({
  ...
  return <div className="mb-8">
      ...
      {data.description && <p className="text-gray-700 dark:text-gray-300 leading-relaxed mb-6 pl-15">{data.description}</p>}
        ...
        {data.rulers.map((ruler: Ruler, i: number) => <div key={i} className="relative group">
            ...
            ">
              ...
              <div className="space-y-2 text-sm">
                {ruler.dynasty && <p className="text-gray-700 dark:text-gray-300">
...
export default function HistoricalTimeline() {
  ...
  if (isLoading && !timeline.title) {
    return <PageLayout metaKey="historical_timeline.meta" title="" breadcrumbs={[{
      labelKey: 'Home',
      href: '/'
    }, {
  ...
  return <PageLayout metaKey="historical_timeline.meta" title={timeline.title} breadcrumbs={[{
    labelKey: 'Home',
    href: '/'
  }, {
    label: 'Historical Timeline'
      ...
      <div id="timeline-content" className="space-y-12">
        ...
        {timeline.sections && timeline.sections.length > 0 && <div className="space-y-8">
            {timeline.sections.map((section: Section, idx: number) => <section key={section.id || idx} id={section.id} className="mb-8">
                ...
                {section.id === 'definitions' && section.items && <div className="flex flex-wrap gap-6">
                    {section.items.map((item: any, i: number) => <div key={i} className="bg-white dark:bg-gray-800 border-l-4 border-amber-500 rounded-lg p-6 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                        ...
                        <div className="space-y-2 text-sm">
                          <p className="text-gray-700 dark:text-gray-300"><strong className="text-amber-800 dark:text-amber-200">{historical_timeline_meta_page.time_period}</strong> {item.timePeriod}</p>
                          <p className="text-gray-700 dark:text-gray-300"><strong className="text-amber-800 dark:text-amber-200">{historical_timeline_meta_page.counting_directio}</strong> {item.countingDirection}</p>
                          <p className="text-gray-700 dark:text-gray-300"><strong className="text-amber-800 dark:text-amber-200">{historical_timeline_meta_page.equivale}</strong> {item.equivalentTo}</p>
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            ...
                            <ul className="space-y-2">
                              {section.notes.map((note: string, i: number) => <li key={i} className="flex items-start gap-2 text-gray-700 dark:text-gray-300">
                                  <span className="text-amber-500 mt-1">•</span>
                ...
                {/* Mapping Section */}
                {section.id === 'mapping-bcad' && section.mapping && <div>
                    <div className="flex gap-4 mb-4">
                      <div className="font-bold flex-1">{historical_timeline_meta_page.old_bc}</div>
                      <div className="font-bold flex-1">{historical_timeline_meta_page.new_bce}</div>
                      {section.mapping.map((map: any, i: number) => <React.Fragment key={i}>
                          <div>{map.old}</div>
                          <div>{map.new}</div>
                        </React.Fragment>)}
                    </div>
                    {section.rules && <div className="mt-4">
                        <strong>{historical_timeline_meta_page.rules}</strong>
                        <ul className="list-disc ml-6">
                          {section.rules.map((rule: string, i: number) => <li key={i}>{rule}</li>)}
                        </ul>
                      </div>}
                  </div>}

                {/* Year Zero Section */}
                {section.id === 'year-zero' && <div>
                    <p className="mb-2"><strong>{historical_timeline_meta_page.exists}</strong> {section.exists ? 'Yes' : 'No'}</p>
                    {section.reason && <div className="mb-4">
                        <strong>{historical_timeline_meta_page.reason}</strong>
                        <ul className="list-disc ml-6">
                          {section.reason.map((r: string, i: number) => <li key={i}>{r}</li>)}
                        </ul>
                      </div>}
                    {section.transition && <p className="mb-2"><strong>{historical_timeline_meta_page.transiti}</strong> {section.transition}</p>}
                    ...
                    {section.implications && <div className="mb-4">
                        <strong>{historical_timeline_meta_page.implicat}</strong>
                        <ul className="list-disc ml-6">
                ...
                {section.id === 'calculations' && <div>
                    {section.rules && <div className="mb-4">
                        <strong>{historical_timeline_meta_page.rules}</strong>
                        <ul className="list-disc ml-6">
                          {section.rules.map((rule: string, i: number) => <li key={i}>{rule}</li>)}
                        </ul>
                    ...
                    {section.centuryRules && <div className="mt-4">
                        <strong>{historical_timeline_meta_page.century_rules}</strong>
                        <ul className="list-disc ml-6">
                          {section.centuryRules.map((rule: string, i: number) => <li key={i}>{rule}</li>)}
                        </ul>
                      </div>}
                  </div>}

                {/* Extended Examples Section */}
                {section.id === 'extended-examples' && section.examples && <div>
                    <div className="space-y-2">
                      {section.examples.map((ex: any, i: number) => <div key={i} className="flex justify-between p-2 border-b">
                          <span>{ex.label}</span>
                ...
                {section.id === 'astronomy-vs-history' && <div className="space-y-4">
                    {section.historySystem && <div className="p-4 border rounded">
                        <h4>{historical_timeline_meta_page.history_system} {section.historySystem.label}</h4>
                        <p><strong>{historical_timeline_meta_page.uses_year}</strong> {section.historySystem.usesYearZero ? 'Yes' : 'No'}</p>
                      </div>}
                    {section.astronomySystem && <div className="p-4 border rounded">
                        <h4>{historical_timeline_meta_page.astronom_system} {section.astronomySystem.label}</h4>
                        <p><strong>{historical_timeline_meta_page.uses_year}</strong> {section.astronomySystem.usesYearZero ? 'Yes' : 'No'}</p>
                        {section.astronomySystem.mapping && <div className="mt-2">
                            <strong>{historical_timeline_meta_page.mapping}</strong>
                            <div className="flex gap-2 mt-2">
                              <div className="font-bold flex-1">{historical_timeline_meta_page.astronom}</div>
                              <div className="font-bold flex-1">{historical_timeline_meta_page.historic}</div>
                              {section.astronomySystem.mapping.map((m: any, i: number) => <React.Fragment key={i}>
        ...
        {/* Diagrams */}
        {timeline.diagrams && timeline.diagrams.ascii && timeline.diagrams.ascii.art && <section className="mb-8">
        ...
        {(timeline.india?.rulers && timeline.india.rulers.length > 0 || timeline.persia?.rulers && timeline.persia.rulers.length > 0 || timeline.rome?.rulers && timeline.rome.rulers.length > 0 || timeline.egypt?.rulers && timeline.egypt.rulers.length > 0 || timeline.china?.rulers && timeline.china.rulers.length > 0 || timeline.greece?.rulers && timeline.greece.rulers.length > 0) && <section className="mb-8">
              <h3>{historical_timeline_meta_page.historic_rulers}</h3>
        ...
        {/* FAQ Section */}
        {timeline.faq && timeline.faq.qa && timeline.faq.qa.length > 0 && <FaqAccordion items={timeline.faq.qa} heading={timeline.faq.heading} />}
      </div>
    </PageLayout>;
}