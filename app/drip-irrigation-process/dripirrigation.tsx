  ...
  const renderSectionContent = (section: SectionData) => (
    <div className="mt-5 space-y-5 text-base leading-7 text-slate-700">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  {Array.isArray(step.details) ? (
                    ...
                    <p className="mt-1 text-sm text-slate-700">{step.details}</p>
                  )}
                </div>
              </div>
            </li>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {section.items.map((item, index) => (
            ...
            >
              ...
              {item.function ? <p className="mt-1 text-sm text-slate-700">{item.function}</p> : null}
              {item.sizingNotes ? <p className="mt-2 text-xs text-slate-600">{item.sizingNotes}</p> : null}
              {item.symptom ? <p className="mt-1 text-sm font-medium text-slate-900">{item.symptom}</p> : null}
              {item.q ? <p className="mt-1 text-sm font-medium text-slate-900">Q. {item.q}</p> : null}
              {item.a ? <p className="mt-1 text-sm text-slate-700">{item.a}</p> : null}
              ...
              {item.fixes?.length ? (
                <p className="mt-1 text-xs text-slate-600">Fixes: {item.fixes.join(', ')}</p>
              ) : null}
            </article>
          ))}
        </div>
        ...
        <div className="space-y-4">
          {section.formulae.map((formula, index) => (
            ...
            >
              ...
              {formula.expr ? (
                <div className="mt-2 overflow-x-auto rounded-lg bg-slate-900 px-3 py-2 text-sm text-emerald-300">
                  {formula.expr}
                </div>
              ) : null}
              {formula.variables ? <p className="mt-2 text-xs text-slate-600">{formula.variables}</p> : null}
              {formula.units ? <p className="mt-1 text-xs text-slate-600">{formula.units}</p> : null}
              {formula.note ? <p className="mt-1 text-xs text-slate-600">{formula.note}</p> : null}
              {formula.example ? <p className="mt-1 text-xs text-slate-600">{formula.example}</p> : null}
            </div>
          ))}
        </div>
      ) : null}
      ...
      {section.workedExample ? (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
          ...
          {section.workedExample.calc?.length ? (
            <ol className="mt-2 list-decimal pl-5 text-sm text-emerald-900">
          ...
          {section.workedExample.result ? (
            <p className="mt-2 text-sm font-medium text-emerald-900">{section.workedExample.result}</p>
          ) : null}
        </div>
      ) : null}
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {section.patterns.map((pattern, index) => (
            ...
            >
              ...
              <p className="mt-1 text-sm text-slate-700">{pattern.description}</p>
              {pattern.pros?.length ? (
                <p className="mt-2 text-xs text-green-700">Pros: {pattern.pros.join(', ')}</p>
              ) : null}
              {pattern.cons?.length ? (
                <p className="mt-1 text-xs text-red-700">Cons: {pattern.cons.join(', ')}</p>
              ) : null}
            </article>
          ))}
        </div>
      ...
      {section.method?.length ? (
        <div>
          <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-900">Method</h3>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {section.terms.map((term, index) => (
            ...
            >
              ...
              <p className="mt-1 text-sm text-slate-700">{term.def}</p>
            </article>
          ))}
        </div>
      ) : null}
              ...
              <ul className="space-y-1">
                {tocItems.map((item) => (
                  <li key={item.id}>
                    <a
                      href={`#${item.id}`}
                      className="block rounded-lg px-2 py-1.5 text-sm text-slate-700 transition hover:bg-emerald-50 hover:text-emerald-800"
                    >
                      {item.label}
                    </a>
                  </li>
