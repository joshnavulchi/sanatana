...
export default function TemplesDestroyedClient() {
  ...
  if (isLoading && !data.title) {
    return <PageLayout metaKey="temples_destroyed" title="" breadcrumbs={[{
      labelKey: 'Home',
      href: '/'
    }, {
      label: 'Temples'
  ...
  return <PageLayout metaKey="temples_destroyed" title={data.title} breadcrumbs={[{
    labelKey: 'Home',
    href: '/'
  }, {
    label: data.title
      ...
      return <div key={section.id || index} className="relative bg-white border-2 border-slate-100 rounded-2xl p-6 md:p-8 mt-12 transform-gpu transition-transform duration-300 hover:-translate-y-1 hover:scale-103 ring-1 ring-slate-100/30 bg-white/85 backdrop-blur-sm group overflow-hidden">
            ...
            <div className="relative z-10 space-y-4">
              ...
              <p className="text-base md:text-md leading-relaxed ">{section.summary || section.event_type?.join?.(', ') || section.period || section.approx_date}</p>
              <ul className="mt-3  space-y-2">
                {section.location && <li><strong>{temples_destroyed_page.location}</strong> {section.location.city_or_district || section.location.state_province || JSON.stringify(section.location)}</li>}
                {section.timeline && <li><strong>{temples_destroyed_page.timeline}</strong> {Array.isArray(section.timeline) ? section.timeline.join(', ') : JSON.stringify(section.timeline)}</li>}
                {section.verification_status && <li><strong>{temples_destroyed_page.verifica}</strong> {section.verification_status}</li>}
              </ul>
            </div>
          </div>;
    })}
      {data.disclaimer && <div className="relative bg-gradient-to-br from-slate-50 to-gray-50 border-l-4 border-slate-400 rounded-lg p-6 md:p-8 mt-12 ring-1 ring-slate-100/30 bg-white/70 backdrop-blur-sm">
          <div className="flex items-start gap-4">
            <span className="text-3xl">⚠️</span>
            <div className="flex-1">
              <h4 className="text-base md:text-md font-bold text-gray-900 mb-2">{temples_destroyed_page.disclaim}</h4>
