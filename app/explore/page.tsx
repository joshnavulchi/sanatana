...
export default async function Page() {
  ...
  const { data } = await fetchContent(locale, ['explore']);
  if (!data) return notFound();

  const title = (data as any)?.title ?? 'Explore';
  ...
  return (
    <>
      ...
      <PageLayout metaKey="explore" title={title} breadcrumbs={[{ label: 'Home', href: '/' }, { label: title }]} className="layout-md">
          ...
          <section>
            ...
            <ul className="space-y-2">
              {children.map((c: any, i: number) => {
                ...
                const href = `/explore/${String(slug)}`;
                return (
                  <li key={i}>
