...
export default function ShaktiPeethasClient() {
  ...
  if (isLoading && !data.title) {
    return <PageLayout metaKey="shakti_peethas" title="" breadcrumbs={[{
      labelKey: 'Home',
      href: '/'
    }, {
  ...
  return <PageLayout metaKey="shakti_peethas" title={data.title} breadcrumbs={[{
    labelKey: 'Home',
    href: '/'
  }, {
    label: data.title
      ...
      <div id="shaktipeethas-content">
        ...
        return <div key={section.id || index} className="relative bg-white border-2 border-rose-100 rounded-2xl p-6 md:p-8 mt-12 transform-gpu transition-transform duration-300 hover:-translate-y-1 hover:scale-105 ring-1 ring-rose-100/30 bg-white/80 backdrop-blur-sm group overflow-hidden">
              ...
              <div className="relative z-10 space-y-4">
                ...
                <p className="text-base md:text-md leading-relaxed ">{section.location || section.body_part || section.shakti || ''}</p>
                <ul className="mt-3  space-y-2">
                  {section.bhairava && <li><strong>{shakti_peethas_page.bhairava}</strong> {section.bhairava}</li>}
                  {section.body_part && <li><strong>{shakti_peethas_page.body_part}</strong> {section.body_part}</li>}
                  {section.location && <li><strong>{shakti_peethas_page.location}</strong> {section.location}</li>}
                  {section.shakti && <li><strong>{shakti_peethas_page.shakti}</strong> {section.shakti}</li>}
                </ul>
              </div>
            </div>;
      })}
        ...
        {data.disclaimer && <div className="relative bg-gradient-to-br from-rose-50 to-pink-50 border-l-4 border-rose-500 rounded-lg p-6 md:p-8 mt-12 ring-1 ring-rose-100/30 bg-white/70 backdrop-blur-sm">
            <div className="flex items-start gap-4">
              <span className="text-3xl">⚠️</span>
              <div className="flex-1">
                <h4 className="text-base md:text-md font-bold text-gray-900 mb-2">{shakti_peethas_page.disclaim}</h4>
