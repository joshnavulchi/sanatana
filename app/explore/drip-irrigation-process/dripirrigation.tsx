...
export default function DropIrrigationClient() {
  ...
  const renderSectionContent = (section: SectionData) => (
    <div className="mt-4 space-y-4 text-md leading-7 text-slate-700">
      {section.summary ? (
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  {Array.isArray(step.details) ? (
                    <ul className="mt-2 list-disc pl-5 text-xl md:text-lg text-slate-700">
                      {step.details.map((detail, detailIndex) => (
                        <li key={`${section.id}-step-${index}-detail-${detailIndex}`}>{detail}</li>
                      ))}
                    </ul>
                  ) : (
                    <p className="mt-1 text-xl md:text-lg text-slate-700">{step.details}</p>
                  )}
                </div>
              </div>
            </li>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {section.items.map((item, index) => (
            ...
            >
              ...
              {item.sizingNotes ? <p className="mt-2 text-md text-slate-600">{item.sizingNotes}</p> : null}
              ...
              {item.fixes?.length ? (
                <p className="mt-1 text-md text-slate-600">Fixes: {item.fixes.join(', ')}</p>
              ) : null}
            </article>
          ))}
        </div>
        ...
        <div className="space-y-4">
          {section.formulae.map((formula, index) => (
            ...
            >
              ...
              {formula.expr ? (
                <div className="mt-2 overflow-x-auto rounded-lg bg-slate-900 px-3 py-2 text-xl md:text-lg text-emerald-300">
                  {formula.expr}
                </div>
              ) : null}
              {formula.variables ? <p className="mt-2 text-md text-slate-600">{formula.variables}</p> : null}
              {formula.units ? <p className="mt-1 text-md text-slate-600">{formula.units}</p> : null}
              {formula.note ? <p className="mt-1 text-md text-slate-600">{formula.note}</p> : null}
              {formula.example ? <p className="mt-1 text-md text-slate-600">{formula.example}</p> : null}
            </div>
          ))}
        </div>
      ) : null}
      ...
      {section.workedExample ? (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
          ...
          {section.workedExample.given?.length ? (
            <ul className="mt-2 list-disc pl-5 text-xl md:text-lg text-emerald-900">
          ...
          {section.workedExample.calc?.length ? (
            <ol className="mt-2 list-decimal pl-5 text-xl md:text-lg text-emerald-900">
          ...
          {section.workedExample.result ? (
            <p className="mt-2 text-xl md:text-lg font-medium text-emerald-900">{section.workedExample.result}</p>
          ) : null}
        </div>
      ) : null}
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {section.patterns.map((pattern, index) => (
            ...
            >
              ...
              {pattern.pros?.length ? (
                <p className="mt-2 text-md text-green-700">Pros: {pattern.pros.join(', ')}</p>
              ) : null}
              {pattern.cons?.length ? (
                <p className="mt-1 text-md text-red-700">Cons: {pattern.cons.join(', ')}</p>
              ) : null}
            </article>
          ))}
        </div>
      ...
      {section.method?.length ? (
        <div>
          <h3 className="text-xl md:text-lg font-semibold uppercase tracking-wide text-slate-900">Method</h3>
      ...
      {section.applicationRateNote ? (
        <p className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xl md:text-lg text-amber-900">
          {section.applicationRateNote}
        </p>
      ) : null}

      {section.sensors?.length ? (
        <ul className="list-disc space-y-1 pl-5 text-xl md:text-lg text-slate-700">
        ...
        <ul className="space-y-2">
          {section.checklist.map((entry, index) => (
            <li
              ...
              className="rounded-xl border border-slate-200 bg-white px-4 py-3 text-xl md:text-lg"
            >
              {entry}
            </li>
          ))}
      ...
      <section className="relative overflow-hidden rounded-3xl border border-emerald-200 bg-gradient-to-br from-emerald-50 via-white to-cyan-50 p-4 shadow-sm md:p-8">
        ...
        <div className="relative">
          {dripIrrigation.hero?.badge ? (
            <span className="inline-flex rounded-full border border-emerald-200 bg-white px-3 py-1 text-md font-semibold uppercase tracking-wider text-emerald-700">
              {dripIrrigation.hero.badge}
            </span>
          ) : null}
          ...
          {dripIrrigation.hero?.subtitle ? (
            <p className="mt-3 max-w-3xl text-md text-slate-700 text-md">
              {dripIrrigation.hero.subtitle}
            </p>
          ) : null}
          {dripIrrigation.description ? (
            <p className="mt-4 max-w-4xl text-xl md:text-lg leading-7 text-slate-600 md:text-md">
              {dripIrrigation.description}
            </p>
          ) : null}
        </div>
              ...
              <ul className="space-y-1">
                {tocItems.map((item) => (
                  <li key={item.id}>
                    <a
                      href={`#${item.id}`}
                      className="block rounded-lg px-2 py-1.5 text-xl md:text-lg text-slate-700 transition hover:bg-emerald-50 hover:text-emerald-800"
                    >
                      {item.label}
                    </a>
                  </li>
        ...
        <div className="space-y-6">
          ...
          {dripIrrigation.diagram ? (
            <section className="rounded-2xl border border-indigo-200 bg-indigo-50 p-4 shadow-sm md:p-6">
              ...
              <div className="mt-4 overflow-x-auto rounded-xl bg-indigo-950 p-4">
                <pre className="whitespace-pre-wrap font-mono text-md leading-7 text-indigo-100">
          ...
          {dripIrrigation.cta ? (
            <section className="rounded-2xl bg-gradient-to-r from-emerald-600 to-cyan-600 p-4 text-white shadow-lg mb-6 md:mb-0">
              ...
              <button
                type="button"
                className="mt-4 items-center rounded-lg bg-white px-4 py-2 text-xl md:text-lg font-semibold text-emerald-700 shadow-sm transition hover:bg-emerald-50 hidden"
              >
