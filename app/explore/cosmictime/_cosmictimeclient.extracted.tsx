...
const cosmictime_page = {
  citable: "Citable",
  concise_structur: "Concise, structured insights from cosmic to human time — organized for clarity and study.",
  research_sections: "Research Sections"
};
...
export default function CosmictimeClient() {
  ...
  const BADGE_BORDER: Record<string, string> = {
    ...
    part1: 'border-amber-200',
    part2: 'border-amber-200',
    part3: 'border-amber-200',
    part4: 'border-amber-200',
    part5: 'border-amber-200',
    part6: 'border-amber-200',
    part7: 'border-amber-200',
    part8: 'border-amber-200',
    part9: 'border-amber-200',
    part10: 'border-amber-200'
  };
  const BADGE_TEXT: Record<string, string> = {
    ...
    part1: 'text-amber-500',
    part2: 'text-amber-500',
    part3: 'text-amber-500',
    part4: 'text-amber-500',
    part5: 'text-amber-500',
    part6: 'text-amber-500',
    part7: 'text-amber-500',
    part8: 'text-amber-500',
    part9: 'text-amber-500',
    part10: 'text-amber-500'
  };
  const SECTIONS: string[] = ['cosmic_evolution', 'planetary_evolution', 'abiogenesis', 'biological_evolution', 'human_evolution_global', 'human_evolution_india', 'timeline', 'part1', 'part2', 'part3', 'part4', 'part5', 'part6', 'part7', 'part8', 'part9', 'part10'];

  // Part-specific visual variants (unique look per part)
  const PART_VARIANTS: Record<string, {
    card: string;
    badge: string;
    title: string;
    subtitle?: string;
    ...
    part1: {
      card: 'bg-gradient-to-br from-amber-50 to-amber-100 border-l-8 border-amber-300 p-6 rounded-2xl shadow-md',
      badge: 'bg-amber-200 text-amber-700',
      title: 'text-amber-800',
      subtitle: 'text-xl md:text-lg text-amber-700'
    },
    part2: {
      card: 'bg-gradient-to-br from-orange-50 to-orange-100 border-l-8 border-orange-300 p-6 rounded-2xl shadow-md',
      badge: 'bg-orange-200 text-orange-700',
      title: 'text-orange-800',
      subtitle: 'text-xl md:text-lg text-orange-700'
    },
    part3: {
      card: 'bg-gradient-to-br from-yellow-50 to-yellow-100 border-l-8 border-yellow-300 p-6 rounded-2xl shadow-md',
      badge: 'bg-yellow-200 text-yellow-700',
      title: 'text-yellow-800',
      subtitle: 'text-xl md:text-lg text-yellow-700'
    },
    part4: {
      card: 'bg-gradient-to-br from-amber-50 to-amber-100 border-l-8 border-amber-300 p-6 rounded-2xl shadow-md',
      badge: 'bg-amber-200 text-amber-700',
    ...
    part5: {
      card: 'bg-gradient-to-br from-emerald-50 to-emerald-100 border-l-8 border-emerald-300 p-6 rounded-2xl shadow-md',
      badge: 'bg-emerald-200 text-emerald-700',
      title: 'text-emerald-800',
      subtitle: 'text-xl md:text-lg text-emerald-700'
    },
    part6: {
      card: 'bg-gradient-to-br from-lime-50 to-lime-100 border-l-8 border-lime-300 p-6 rounded-2xl shadow-md',
      badge: 'bg-lime-200 text-lime-700',
      title: 'text-lime-800',
      subtitle: 'text-xl md:text-lg text-lime-700'
    },
    part7: {
      card: 'bg-gradient-to-br from-amber-50 to-amber-100 border-l-8 border-amber-300 p-6 rounded-2xl shadow-md',
      badge: 'bg-amber-200 text-amber-700',
  ...
  if (isLoading && !data.title) {
    return <PageLayout metaKey="cosmictime" title="" breadcrumbs={[{
      labelKey: "Home",
      href: "/"
    }, {
      label: "Cosmic Time"
  ...
  return <PageLayout metaKey="cosmictime" title={data.title} breadcrumbs={[{
    labelKey: "Home",
    href: "/"
  }, {
    label: data.title
          ...
          <main className="w-full lg:w-3/4 hidden!">
            <header className="px-6 py-10 rounded-3xl bg-gradient-to-r from-amber-50 via-orange-50 to-yellow-50 border border-orange-100 shadow-sm">
              <div className="max-w-5xl mx-auto text-center">
                ...
                <p className="text-amber-900 max-w-3xl mx-auto">{cosmictime_page.concise_structur}</p>
                <div className="mt-4 flex justify-center gap-3">
                  <span className="inline-flex items-center gap-2 bg-white/80 border border-amber-100 px-3 py-1 rounded-full text-xl md:text-lg text-amber-800">{cosmictime_page.research_sections}</span>
                  <span className="inline-flex items-center gap-2 bg-white/80 border border-amber-100 px-3 py-1 rounded-full text-xl md:text-lg text-amber-800">{cosmictime_page.citable}</span>
                </div>
              </div>
            </header>
            ...
            <section className="mt-8 grid gap-6">
              ...
              if (isPart) {
                ...
                return <article key={sectionKey} className={variant.card}>
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          <h3 className={`text-base md:text-md font-semibold ${variant.title} mb-2`}>{`Part ${sectionKey.replace('part', '')}: ${section?.dataset_metadata?.title || sectionKey.replace(/_/g, ' ')}`}</h3>
                          {subtitle && <div className={`${variant.subtitle} mb-3`}>{subtitle}</div>}
                          <div className="space-y-3 text-amber-900 text-xl md:text-lg">
              ...
              return <article key={sectionKey} className="bg-white border-l-4 rounded-2xl p-6 shadow-sm hover:shadow-md transform hover:-translate-y-1 transition">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <h3 className="text-base md:text-md font-semibold text-orange-800 mb-2">{sectionKey.replace(/_/g, ' ')}</h3>
                        <div className="space-y-3 text-amber-900 text-xl md:text-lg">
        ...
        return Object.entries(section).map(([k, v]) => {
          ...
          return <div key={k}>
                ...
                <div className="ml-3 mt-2">
                  {isSpecial ? Array.isArray(v) ? <ul className="list-disc pl-6 space-y-1 text-amber-900">
                        {v.map((item: any, i: number) => <li key={i}>{item}</li>)}
                      </ul> : <p className="text-amber-900">{String(v)}</p> : renderSectionContent(v)}
                </div>
              </div>;
        });
      })()}
...
function renderTimeline(section: any) {
  ...
  if (!section) return null;
  if (Array.isArray(section)) {
    return <ol className="space-y-8">
        {section.map((item: any, idx: number) => <li key={idx} className={`relative flex flex-col md:flex-row items-start gap-4 ${idx % 2 === 1 ? 'md:flex-row-reverse' : ''}`}>
  ...
  if (section && typeof section === 'object') {
    return <ol className="space-y-8">
        {Object.entries(section).map(([k, v], idx) => <li key={k} className={`relative flex flex-col md:flex-row items-start gap-4 ${idx % 2 === 1 ? 'md:flex-row-reverse' : ''}`}>
            <div className="md:w-1/12 flex justify-center">
              <div className="w-3 h-3 rounded-full bg-amber-400 border border-white mt-2" />
            </div>
            <div className="md:w-11/12">
