...
export default function JyotirlingsClient() {
  ...
  return <PageLayout metaKey="jyotirlings" title={data.title} breadcrumbs={[{
    ...
    label: data.title
      ...
      return <div key={section.id || index} className="relative bg-white border-l-4 border-amber-300 rounded-2xl p-6 md:p-8 mt-12 shadow-xl hover:shadow-2xl transition-all duration-500 group overflow-hidden">
      ...
      {data.disclaimer && <div className="relative bg-gradient-to-br from-yellow-50 to-amber-50 border-l-4 border-amber-600 rounded-lg p-6 md:p-8 mt-12 ring-1 ring-yellow-100/30 bg-white/70 backdrop-blur-sm">
          <div className="flex items-start gap-4">
