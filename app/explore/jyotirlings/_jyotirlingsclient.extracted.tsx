...
export default function JyotirlingsClient() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const items = Array.isArray(container?.lingas) ? container.lingas : Array.isArray(container?.items) ? container.items : [];
  ...
  if (isLoading && !data.title) {
    return <PageLayout metaKey="jyotirlings" title="" breadcrumbs={[{
      labelKey: 'Home',
      href: '/'
    }, {
      label: 'Jyotirlings'
  ...
  return <PageLayout metaKey="jyotirlings" title={data.title} breadcrumbs={[{
    labelKey: 'Home',
    href: '/'
  }, {
    label: data.title
    ...
    <div id="jyotirlings-content" className="relative px-3 md:px-6 py-4 md:py-16 bg-gradient-to-br from-yellow-50 via-amber-100 to-yellow-50 rounded-2xl border-l-4 border-amber-400 shadow-2xl overflow-hidden group">
    ...
    {data.items.map((section: any, index: number) => {
      ...
      return <div key={section.id || index} className="relative bg-white border-l-4 border-amber-300 rounded-2xl p-4 md:p-8 mt-12 shadow-xl hover:shadow-2xl transition-all duration-500 group overflow-hidden">
    ...
    {data.disclaimer && <div className="relative bg-gradient-to-br from-yellow-50 to-amber-50 border-l-4 border-amber-600 rounded-lg p-4 md:p-8 mt-12 ring-1 ring-yellow-100/30 bg-white/70 backdrop-blur-sm">
      <div className="flex items-start gap-4">
        <span className="text-3xl">⚠️</span>
        <div className="flex-1">
          <h4 className="text-base md:text-md font-bold text-gray-900 mb-2">{jyotirlings_page.disclaim}</h4>
