...
import IndiaMapAnimated from '@components/worldmap/indiamap';
const rivers_connecting_page = {
  end: "End:",
  id: "ID:",
  phase_types: "Phase Types Legend",
  phases: "phases",
  project_timeline: "Project Timeline 2026–2040",
  project_timeline_1: "Project Timeline Complete",
  river_interlin: "River interlinking phased implementation roadmap",
  start: "Start:"
};
...
export default function RiversConnectingClient() {
  ...
  if (isLoading && !data.title) {
    return <PageLayout metaKey="rivers_connecting" title="" breadcrumbs={[{
      labelKey: 'Home',
      href: '/'
    }, {
      label: 'Rivers Connecting'
    }]} className="layout-md">
  ...
  return <>
      <IndiaMapAnimated />
      <PageLayout metaKey="rivers_connecting" title={data.title} breadcrumbs={[{
      labelKey: 'Home',
      href: '/'
    }, {
      label: data.title
        ...
        {data.content && <div className="relative bg-white border-l-4 border-blue-300 rounded-2xl p-6 md:p-8 mt-12 shadow-xl">
        ...
        {data.ganttData && data.ganttData.length > 0 && <div className="relative bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50 rounded-3xl p-8 md:p-12 mt-16 shadow-2xl overflow-hidden">
            ...
            <div className="relative z-10 mb-12">
              <div className="flex items-center gap-4 mb-4">
                ...
                <h3 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent"> {rivers_connecting_page.project_timeline} </h3>
                <div className="h-1 flex-1 bg-gradient-to-r from-cyan-400 to-transparent rounded-full" />
              </div>
              <p className="text-gray-600 ml-20">{rivers_connecting_page.river_interlin}</p>
            </div>
            ...
            const colors = [{
              ...
              light: 'from-fuchsia-100 to-purple-100',
              border: 'border-fuchsia-400',
              accent: 'bg-fuchsia-500'
            }];
            const color = colors[index % colors.length];
            return <div key={project.id || index} className="relative group">
                    ...
                    <div className="flex items-start gap-4 mb-6">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 text-base md:text-md text-gray-500">
                          <span className="px-2 py-1 bg-white rounded-full shadow-sm">{project.phases?.length || 0} {rivers_connecting_page.phases}</span>
                          <span>•</span>
                          <span>{rivers_connecting_page.id} {project.id}</span>
                        </div>
                      </div>
                    </div>
                  ...
                  const getPhaseStyle = (type: string) => {
                    switch (type) {
                      ...
                      case 'milestone':
                        return {
                          bg: 'bg-gradient-to-r from-purple-400 to-fuchsia-400',
                          icon: '🎯',
                  ...
                  return <div key={phaseIndex} className="relative group/phase">
                            {/* Connection dot */}
                            ...
                            <div className={`relative bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border-l-4 ${color.border} group-hover/phase:translate-x-2`}>
                              ...
                              <div className="relative p-5">
                                <div className="flex items-start justify-between gap-4">
                                  <div className="flex-1">
                                    <div className="flex items-center gap-3 mb-2">
                                      ...
                                      <h5 className={`text-xl md:text-lg font-bold ${phaseStyle.textColor}`}>
                                        {phase.label}
                                      </h5>
                                    </div>
                                    <div className="flex flex-wrap items-center gap-3 text-base md:text-md">
                                      <div className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-lg">
                                        <span className="text-gray-500">{rivers_connecting_page.start}</span>
                                        <span className="font-semibold text-gray-700">{phase.start}</span>
                                      </div>
                                      <span className="text-gray-400">→</span>
                                      <div className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-lg">
                                        <span className="text-gray-500">{rivers_connecting_page.end}</span>
                                ...
                                <div className="mt-4 h-2 bg-gray-100 rounded-full overflow-hidden">
                                  <div className={`h-full ${phaseStyle.bg} rounded-full animate-pulse`} style={{
                            width: '100%'
                          }} />
                                </div>
                              </div>
                            ...
                            {!isLastPhase && <div className="absolute -left-[2.1rem] top-8 bottom-0 w-0.5 bg-gradient-to-b from-blue-300 via-cyan-300 to-transparent" />}
                          </div>;
                })}
                    </div>
                    ...
                    <div className="ml-7 pl-8 mt-4">
                      <div className={`inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r ${color.primary} text-white rounded-full shadow-lg text-base md:text-md font-semibold`}>
                        <span>✓</span>
                        <span>{rivers_connecting_page.project_timeline_1}</span>
                      </div>
                    </div>
                  </div>;
          })}
            ...
            <div className="relative z-10 mt-16 pt-8 border-t-2 border-dashed border-gray-300">
              <h4 className="text-base md:text-md font-bold text-gray-700 mb-4 uppercase tracking-wide">{rivers_connecting_page.phase_types}</h4>
              ...
              color: 'from-slate-400 to-gray-400'
            }].map((item, idx) => <div key={idx} className="flex items-center gap-3 bg-white rounded-lg p-3 shadow-sm hover:shadow-md transition-shadow">
                    ...
                    <span className="text-base md:text-md font-medium text-gray-700">{item.type}</span>
                  </div>)}
              </div>
            </div>
          </div>}
