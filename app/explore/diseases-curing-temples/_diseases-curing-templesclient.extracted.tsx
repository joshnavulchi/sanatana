...
const diseases_curing_temples_page = {
  disclaim: "Disclaimer"
};
...
export default function DiseasesCuringTemplesClient() {
  ...
  if (isLoading && !data.title) {
    return <PageLayout metaKey="diseases_curing_temples" title="" breadcrumbs={[{
      labelKey: 'Home',
      href: '/'
    }, {
      label: 'Temples'
  ...
  return <PageLayout metaKey="diseases_curing_temples" title={data.title} breadcrumbs={[{
    labelKey: 'Home',
    href: '/'
  }, {
    label: data.title
      ...
      return <div key={section.id || index} className="relative bg-white border-2 border-emerald-100 rounded-2xl p-4 md:p-8 mt-12 transform-gpu transition-transform duration-300 hover:-translate-y-1 hover:scale-105 ring-1 ring-emerald-100/30 bg-white/80 backdrop-blur-sm group overflow-hidden">
            ...
            <div className="relative z-10 space-y-4">
              ...
              {section?.traditional_belief_about_diseases && section.traditional_belief_about_diseases.length > 0 && <ul className="space-y-3 ">
                  {section.traditional_belief_about_diseases.map((text: string, idx: number) => <li key={idx} className="relative flex items-start gap-3 leading-relaxed">
                      <span className="flex-shrink-0 w-2 h-2 mt-2 bg-gradient-to-br from-emerald-500 to-green-400 rounded-full" />
                      <span className="flex-1">{text}</span>
                    </li>)}
                </ul>}
              {!section?.traditional_belief_about_diseases && (section.summary || section.text) && <p className="text-base md:text-md leading-relaxed ">{section.summary || section.text}</p>}
            </div>
          </div>;
    })}
      {data.disclaimer && <div className="relative bg-gradient-to-br from-emerald-50 to-green-50 border-l-4 border-emerald-500 rounded-lg p-4 md:p-8 mt-12 ring-1 ring-emerald-100/30 bg-white/70 backdrop-blur-sm">
          <div className="flex items-start gap-4">
            <span className="text-3xl">⚠️</span>
            <div className="flex-1">
              <h4 className="text-base md:text-md font-bold text-gray-900 mb-2">{diseases_curing_temples_page.disclaim}</h4>
