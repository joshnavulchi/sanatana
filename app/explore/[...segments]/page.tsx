...
export default async function Page({ params }: Props) {
  ...
  const segments = ['explore', ...rawSegments];
  ...
  const crumbs = [{ label: 'Home', href: '/' }, { label: 'Explore', href: '/explore' }];
  let accum: string[] = [];
  for (const s of rawSegments) {
    accum.push(s);
    crumbs.push({ label: s, href: `/explore/${accum.join('/')}` });
  }

  return (
    <>
      <StructuredData metaKey="explore" />
      <PageLayout metaKey="explore" title={title} breadcrumbs={crumbs} className="layout-md">
          ...
          <section>
            ...
            <ul className="space-y-2">
              {children.map((c: any, i: number) => {
                ...
                const href = `/explore/${[...rawSegments, String(slug)].join('/')}`;
                return (
                  <li key={i}>
