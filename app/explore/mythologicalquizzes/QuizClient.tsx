    ...
    return (
      <div>
        ...
        <div className="flex items-center justify-between my-6">
          <div className="text-base md:text-md">{ns?.yourScore || 'Your score:'} <strong>{score}</strong> / {qList.length}</div>
          <div className="text-base md:text-md">{ns?.timeTaken || 'Time taken:'} {fmtTime(10 * 60 - timeLeft)}</div>
        </div>
        <div>
          {qList.map((q, idx) => (
            <div key={q.id}>
              <div className="text-base md:text-md font-semibold">{idx + 1}. {q.question}</div>
