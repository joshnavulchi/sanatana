  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-1 flex-col p-6 sm:p-8">
        ...
        {/* Subtitle */}
        <p className="mt-2 text-base sm:text-lg font-semibold uppercase tracking-widest text-indigo-700">
          {subtitle}
        </p>
        ...
        {description && description.length > 0 && (
          <ul className="mt-6 flex flex-col gap-3">
            {description.map((point, i) => (
              <li key={i} className="flex items-start gap-3 text-base sm:text-lg leading-relaxed text-gray-700 dark:text-gray-200">
        ...
        <div className="mt-auto pt-6">
          <span className={`inline-flex items-center gap-2 rounded-xl border-2 border-indigo-200 bg-gradient-to-r from-amber-50 via-pink-50 to-indigo-50 px-3 py-2 text-base sm:text-lg font-bold tracking-widest text-indigo-700 shadow-md transition-all duration-300 group-hover:shadow-xl group-hover:scale-105`}>
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl px-4">
        ...
        <div className={`mt-10 transition-all duration-1000 ease-out delay-500 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
          ...
          <div className="mb-6 flex items-center justify-center gap-3">
            ...
            <span className="text-base sm:text-lg font-black uppercase tracking-[0.35em] text-[#92400e]">
              ✦ Cycle of Time ✦
            </span>
          ...
          {earthAgeComparisonNote && (
            <div className="mx-auto max-w-5xl bg-black/5 rounded-md border border-[#d8a25a]/15 p-4 text-center shadow-[0_2px_5px_rgba(166,61,23,0.10)]">
              <p className="bg-linear-to-r from-[#7a2e1f] via-[#9a3412] to-[#7a2e1f] bg-clip-text text-base sm:text-lg leading-relaxed">
                {earthAgeComparisonNote}
              </p>
            </div>
          )}
          ...
          {scalingComment && (
            <p className="mx-auto mt-5 max-w-4xl text-center text-base sm:text-lg leading-relaxed text-[#5b2d12]">
              {scalingComment}
            </p>
          )}
        </div>
