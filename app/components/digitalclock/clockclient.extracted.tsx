...
export default function DigitalClock({
  ...
  useEffect(() => {
    ...
    timeoutId = setTimeout(() => {
      try {
        ...
        let phaseName = '';
        if (phase <= 0.03 || phase >= 0.97) phaseName = 'New Moon (Amavasya)';else if (phase < 0.25) phaseName = 'Waxing Crescent';else if (phase < 0.27) phaseName = 'First Quarter (Half Moon)';else if (phase < 0.5) phaseName = 'Waxing Gibbous';else if (phase >= 0.48 && phase <= 0.52) phaseName = 'Full Moon (Purnima)';else if (phase < 0.75) phaseName = 'Waning Gibbous';else if (phase < 0.77) phaseName = 'Last Quarter (Half Moon)';else phaseName = 'Waning Crescent';
        ...
        if (sr && ss) {
          ...
          newRahu = {
            start: DateTime.fromJSDate(rStart).setZone(zone).toLocaleString(DateTime.TIME_SIMPLE),
            end: DateTime.fromJSDate(rEnd).setZone(zone).toLocaleString(DateTime.TIME_SIMPLE)
          };
          newYama = {
            start: DateTime.fromJSDate(yStart).setZone(zone).toLocaleString(DateTime.TIME_SIMPLE),
            end: DateTime.fromJSDate(yEnd).setZone(zone).toLocaleString(DateTime.TIME_SIMPLE)
          };
        }
  ...
  return <div className="fixed z-9 top-32 left-4 max-w-xs md:max-w-sm" style={{
    pointerEvents: 'none'
  }}>
      <div className="relative">
        ...
        zIndex: 10
          ...
          <span className="inline-block transform transition-transform duration-300">
            {visible ? <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className={`transition-all duration-300 ${visible ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'} bg-white/90 dark:bg-gray-900/90 shadow-2xl rounded-xl border border-gray-200 dark:border-gray-700 p-4 flex flex-col items-end`} style={{
            ...
            <div className="flex items-baseline justify-end gap-2">
              <div className="flex items-baseline gap-1">
                <div className="flex text-3xl md:text-4xl font-mono font-bold text-blue-700 dark:text-blue-300 select-none">
                  {String(hours12).split('').map((d, i) => <span key={`h${i}`}>{d}</span>)}
                  <span>:</span>
                  {pad(minutes).split('').map((d, i) => <span key={`m${i}`}>{d}</span>)}
                  {showSeconds && <>
                      <span>:</span>
                      {pad(seconds).split('').map((d, i) => <span key={`s${i}`}>{d}</span>)}
                    </>}
                </div>
                <span className="ml-1 text-base text-gray-600 dark:text-gray-300 font-semibold">{ampm}</span>
              </div>
            </div>
            {latLng && <div className="text-xs flex flex-col text-right gap-2 mt-2">
                {sunrise && <div className="flex items-center gap-2 justify-end bg-gradient-to-r from-yellow-200 via-yellow-100 to-white dark:from-yellow-900 dark:via-yellow-800 dark:to-gray-900 rounded-lg px-2 py-1 shadow-sm mb-1">
                    ...
                    <span className="font-semibold text-yellow-700 dark:text-yellow-200">{page.sunrise}</span>
                    <span className="font-mono text-xs text-yellow-800 dark:text-yellow-100">{sunrise}</span>
                  </div>}
                {sunset && <div className="flex items-center gap-2 justify-end bg-gradient-to-r from-indigo-200 via-purple-100 to-white dark:from-indigo-900 dark:via-purple-800 dark:to-gray-900 rounded-lg px-2 py-1 shadow-sm mb-1">
                    ...
                    <span className="font-semibold text-purple-700 dark:text-purple-200">{page.sunset}</span>
                    <span className="font-mono text-xs text-purple-800 dark:text-purple-100">{sunset}</span>
                  </div>}
                {tithi && <div><span className="font-semibold">{page.lunar_day}</span> {tithi}</div>}
                {nakshatra && <div><span className="font-semibold">{page.nakshatr}</span> {nakshatra}</div>}
                {moonPhase && <div className="flex items-center gap-1 justify-end">
                    <span>{page.moon}</span>
                ...
                {rahu && <div><span className="font-semibold">{page.rahu_kaal}</span> {rahu.start} - {rahu.end}</div>}
                {yama && <div><span className="font-semibold">{page.yamagand}</span> {yama.start} - {yama.end}</div>}
              </div>}
          </div>
        </div>
      </div>
