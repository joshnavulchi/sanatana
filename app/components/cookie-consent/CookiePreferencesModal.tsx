    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center pointer-events-auto">
      ...
      <div className="relative bg-gradient-to-br from-white via-blue-50 to-blue-100 border-2 border-blue-400 shadow-2xl rounded-2xl max-w-lg md:max-w-4xl w-full mx-4 p-6 flex flex-col gap-4 animate-fadeInUp">
        <div className="flex items-center justify-between mb-2">
          <span className="text-md md:text-lg font-bold text-blue-700">Cookie Preferences</span>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="md:w-2/3 w-full">
            <div className="mb-2">
              <span className="text-md md:text-lg font-semibold text-blue-700">{TABS.find(t => t.id === active)?.title}</span>
              <div className="text-md md:text-lg md:text-xs md:text-base text-gray-500">{TABS.find(t => t.id === active)?.description}</div>
            </div>
            {active === 'your-privacy' && (
              <div className="bg-blue-50 rounded-lg p-3 text-xs md:text-base ">
            ...
            {active === 'strictly-necessary' && (
              <div className="bg-gray-100 rounded-lg p-3 text-md md:text-lg flex items-center gap-2">
            ...
            {active !== 'your-privacy' && active !== 'strictly-necessary' && (
              <div className="flex items-center gap-2 bg-blue-50 rounded-lg p-3">
                ...
                />
                <span className="text-xs md:text-base">Enable {TABS.find(t => t.id === active)?.title}</span>
              </div>
            )}
            {active === 'targeting' && (
