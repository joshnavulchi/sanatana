            ...
            <div className="relative z-10 p-6 md:p-8">
              <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
                ...
                <div className="flex-1">
                  <h3 className="text-xl md:text-lg font-bold text-gray-900 mb-2 flex items-center gap-2">
                    <span className="md:hidden text-2xl">🍪</span> {page.cookie_settings} </h3>
                  <div className="text-sm leading-relaxed">
                    {(() => {
                    const paragraph = localeObj?.cookieconsent?.paragraph || '';
                    return <div dangerouslySetInnerHTML={{
                      __html: paragraph.replace('{cookiePolicyLink}', `<a class="text-amber-800 hover:text-orange-700 underline underline-offset-2 transition-colors" href="/our-cookie-policy">${localeObj?.cookieconsent?.cookiepolicy || 'Cookie Policy'}</a>`).replace('{privacyPolicyLink}', `<a class="text-amber-800 hover:text-orange-700 underline underline-offset-2 transition-colors" href="/our-privacy-policy">${localeObj?.cookieconsent?.privacypolicy || 'Privacy Policy'}</a>`).replace('{managerLabel}', localeObj?.cookieconsent?.managerbutton || 'Cookie Preferences').replace('{acceptAllLabel}', localeObj?.cookieconsent?.acceptall || 'Accept all')
                    }} />;
                  })()}
                  </div>
                </div>
