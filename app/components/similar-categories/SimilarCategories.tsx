...
export default function SimilarCategories({
  ...
  if (categories.length === 0) {
    return (
      <aside className="relative overflow-hidden rounded-2xl border-2 border-amber-300 bg-linear-to-b from-amber-50 via-orange-50 to-stone-50 p-5 shadow-[0_10px_30px_rgba(120,53,15,0.12)]">
        <div className="pointer-events-none absolute inset-x-5 top-4 h-px bg-linear-to-r from-transparent via-amber-500/70 to-transparent text-lg sm:text-base leading-relaxed font-normal" />
        <div className="pointer-events-none absolute inset-x-5 bottom-4 h-px bg-linear-to-r from-transparent via-amber-500/70 to-transparent text-lg sm:text-base leading-relaxed font-normal" />
        <h5 className="mb-2 text-xl md:text-lg sm:text-base font-semibold tracking-wide text-amber-900">{title}</h5>
        <p className="text-amber-800/90 text-lg sm:text-base leading-relaxed mb-4 font-normal">
  ...
  return (
    <aside className="relative overflow-hidden rounded-2xl border-2 border-amber-300 bg-linear-to-b from-amber-50 via-orange-50 to-stone-50 p-5 shadow-[0_10px_30px_rgba(120,53,15,0.12)]">
      <div className="pointer-events-none absolute inset-x-5 top-4 h-px bg-linear-to-r from-transparent via-amber-500/70 to-transparent text-lg sm:text-base leading-relaxed font-normal" />
      <div className="pointer-events-none absolute inset-x-5 bottom-4 h-px bg-linear-to-r from-transparent via-amber-500/70 to-transparent text-lg sm:text-base leading-relaxed font-normal" />
      <div className="rounded-xl border border-amber-200 bg-amber-100/70 px-4 py-3 text-lg sm:text-base leading-relaxed font-normal">
        <h5 className="text-xl md:text-lg sm:text-base font-semibold tracking-wide text-amber-900">{title}</h5>
        <p className="text-amber-800 text-lg sm:text-base leading-relaxed mb-4 font-normal">Sacred pathways to explore related wisdom.</p>
      </div>
              ...
              <ul className="space-y-2 list-disc pl-5 text-lg sm:text-base leading-relaxed">
                {(expandedByCategory[category.key] ? category.links : category.links.slice(0, INITIAL_VISIBLE_LINKS)).map((link) => (
                  <li key={link.key} className="mb-2">
                    ...
                    >
                      <span aria-hidden="true" className="h-1.5 w-1.5 shrink-0 rounded-full bg-amber-500 text-lg sm:text-base leading-relaxed font-normal" />
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
              <div className="flex flex-wrap items-center gap-2 border-t border-amber-200/80 pt-3 text-lg sm:text-base leading-relaxed font-normal">
                {category.links.length > INITIAL_VISIBLE_LINKS && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-full border border-amber-300 bg-amber-50 px-3 py-1.5 text-lg sm:text-base font-semibold tracking-wide text-amber-900 transition-colors hover:bg-amber-100"
                  >
                ...
                <Link
                  href={toCategoryHref(category.key, CATEGORY_CONFIG[category.key]?.basePath ?? '')}
                  className="inline-flex items-center gap-1.5 rounded-full border border-orange-300 bg-orange-100 px-3 py-1.5 text-lg sm:text-base font-semibold tracking-wide text-orange-900 transition-colors hover:bg-orange-200"
                >
                  View all
