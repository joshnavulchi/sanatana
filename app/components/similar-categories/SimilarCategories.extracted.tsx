...
export default function SimilarCategories({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const extractedCategories: Array<{
          key: string;
          title: string;
          links: Array<{
            key: string;
  ...
  if (categories.length === 0) {
    return <aside className="p-4">
        <h5>{title}</h5>
        <p>{page.loading_categori}</p>
      </aside>;
  }
  return <aside>
      ...
      <div>
        ...
        return <div key={category.key} className="shadown-sm p-4">
              ...
              <ul>
                {category.links.map(link => <li key={link.key}>
                    <Link href={link.href}>
                      {link.label}
                    </Link>
                  </li>)}
