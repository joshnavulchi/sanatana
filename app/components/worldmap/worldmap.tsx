...
const WorldMap = () => {
  ...
  return (
    <div className="w-full min-h-[200px] m-0 p-0 overflow-hidden relative text-lg sm:text-base leading-relaxed font-normal">
      <style>{labelStyle}</style>
      <div className="absolute top-5 left-0 z-10 bg-white/90 rounded-sm px-3 py-1 mx-3 shadow-md text-lg sm:text-base leading-relaxed font-normal">
        <label htmlFor="era-select" className="text-lg sm:text-base font-semibold mr-2">Geological Era:</label>
        <select id="era-select" value={era} onChange={e => setEra(e.target.value)} className="text-lg sm:text-base px-2 py-1 rounded">
          {ERA_OPTIONS.map(opt => (
      ...
      {error && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white bg-opacity-95 text-red-700 px-4 py-3 rounded shadow-lg z-50 text-lg sm:text-base leading-relaxed font-normal">
          {error}
        </div>
      )}
    </div>
