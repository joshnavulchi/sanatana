...
function pickGeometryObject(world: any) {
  ...
  const firstGC = Object.values(world.objects).find((o: any) => o && o.type === "GeometryCollection");
  return firstGC || null;
}
const WorldMap = () => {
  ...
  const render = useCallback((features: any[], width: number, height: number) => {
    ...
    const lg = defs.append("linearGradient").attr("id", "bg-gradient").attr("x1", "0%").attr("y1", "0%").attr("x2", "100%").attr("y2", "100%");
    ...
    const projection = d3.geoNaturalEarth1().fitExtent([[0, 0], [width, height]], {
      type: "Sphere"
    });
    const path = d3.geoPath().projection(projection);

    // Land/ country paths
    svg.append("g").selectAll("path").data(features).enter().append("path").attr("d", path as any).attr("fill", "#a3d9a5").attr("stroke", "#fff").attr("stroke-width", 0.1).attr("opacity", 0.93).style("filter", "drop-shadow(0 2px 8px #0008)").on("mouseover", function () {
    ...
    svg.append("g").selectAll("text").data(features).enter().append("text").attr("class", "wm-country-label").attr("x", d => path.centroid(d as any)[0]).attr("y", d => path.centroid(d as any)[1]).text((d: any) => d.properties?.iso_a2 || d.properties?.code || d.properties?.name?.slice(0, 2) || "?").attr("text-anchor", "middle").attr("alignment-baseline", "middle").attr("font-size", 10).attr("font-weight", 400).attr("fill", "#fff").attr("stroke", "#fff").attr("stroke-width", 0.0).attr("pointer-events", "none");
  }, []);
  useEffect(() => {
    ...
    (async () => {
      try {
        const world: any = await d3.json(eraFile, {
  ...
  return <div style={{
    ...
    position: "relative"
      ...
      boxShadow: '0 2px 8px #0002'
        ...
        <label htmlFor="era-select" style={{
        fontSize: 14,
        fontWeight: 600,
        marginRight: 4
      }}>{page.geologic_era}</label>
        <select id="era-select" value={era} onChange={e => setEra(e.target.value)} style={{
        fontSize: 14,
        padding: '2px 4px',
        borderRadius: 3
      }}>
