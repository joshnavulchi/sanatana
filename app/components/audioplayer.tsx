  ...
  return (
    <div className={`fixed z-9 top-25 md:top-24 left-2 bg-gradient-to-br from-blue-500 via-purple-500 to-pink-500 hover:from-blue-600 hover:to-pink-600 text-white shadow-lg rounded-full tracking-widest border-2 border-white  focus:outline-none focus:ring-4 focus:ring-pink-300 transition-all duration-300 flex cursor-pointer`}>
      ...
      <div className="gap-4 items-center hidden">
        {playlist.length > 1 && (
          <button
            ...
            onClick={handlePrev}
            title="Previous"
            className="px-3 py-2 rounded-lg border border-indigo-200 bg-white text-base text-indigo-700 hover:bg-indigo-50"
          >⏮</button>
        )}
        <button
          type="button"
          ...
          className={`px-3 py-2 rounded-lg border border-indigo-200 bg-white text-indigo-700 text-base ${isLoop ? 'bg-indigo-100 text-indigo-900' : ''}`}
        >⟲</button>
        <button
          type="button"
          ...
          className={`px-3 py-2 rounded-lg border border-red-200 bg-white text-red-700 text-base ${isMuted ? 'bg-red-100 text-red-900' : ''}`}
        >{isMuted ? "🔇" : "🔊"}</button>
        {playlist.length > 1 && (
          <button
            type="button"
      ...
      <div className="items-center gap-2 hidden">
        <span className="text-gray-500 text-base">Vol</span>
        <input
          type="range"
          min={0}
          max={1}
