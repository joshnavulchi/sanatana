...
const page = {
  truth_alone: "Truth Alone Triumphs"
};
...
export default function HeroSection({
  ...
  return <div className="relative w-full min-h-[600px] px-3 overflow-hidden">
      {/* Background Images with Overlay */}
      <div className="absolute inset-0">
        <Image className="block md:hidden" src="/images/home/mobile-hero.png" alt="Sanātana Dharma hero background" fill sizes="100vw" priority quality={90} style={{
        objectFit: 'cover',
        backgroundPosition: 'top center'
      }} unoptimized />
        {/* Desktop: video + fallback image (added video element) */}
        <div className="hidden md:block absolute inset-0">
          <video ref={videoRefDesktop} src="/videos/kurushetra.mp4" preload="auto" playsInline muted loop autoPlay className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-700 ${isVideoReady ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} />
          <Image className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-700 ${isVideoReady ? 'opacity-0' : 'opacity-100'}`} src={hero?.desktopImage ?? '/images/home/hero.png'} alt="Sanātana Dharma hero background" fill sizes="100vw" priority quality={90} style={{
          objectFit: 'cover',
          backgroundPosition: 'top center'
        }} unoptimized />
        </div>
      ...
      <div className="absolute bottom-20 left-10 w-40 h-40 bg-orange-500/10 rounded-full blur-3xl animate-pulse" style={{
      animationDelay: '1s'
    }} />
            ...
            <div className="relative">
              ...
              <div className="relative bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-lg rounded-3xl p-6 shadow-2xl transform hover:scale-105 transition-transform duration-500">
                ...
                <div className="text-center space-y-6">
                  ...
                  <p className="text-amber-200 text-xl md:text-lg">{page.truth_alone}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className={`w-full transition-all text-center duration-1000 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10'}`}>
            <div className="flex flex-col items-center md:mx-auto md:max-w-2xl">
              ...
              <div className="flex gap-3">
                ...
                <div className="w-2 h-2 rounded-full bg-orange-400 animate-pulse" style={{
                animationDelay: '150ms'
              }} />
                <div className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" style={{
            ...
            <div className="flex flex-col md:mx-auto md:max-w-3xl">
              ...
              <div className="w-full text-center flex flex-col md:flex-row md:justify-center gap-4 mt-6">
                <Link href={hero?.primarycta?.link ? `/${hero.primarycta.link}` : '/scriptures'} className="group relative md:inline-flex px-8 py-3 bg-gradient-to-r from-amber-500 to-orange-600
                ...
                <Link href={hero?.secondarycta?.link ? `/${hero.secondarycta.link}` : '/sanatanadharma'} className="group md:inline-flex px-8 py-3 bg-white/10 backdrop-blur-md
