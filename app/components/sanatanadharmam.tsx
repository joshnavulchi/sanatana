        ...
        {sections.map((section, sectionIndex) => (
          ...
          >
            <div className="text-center">
              ...
              <p className="text-md sm:text-base text-gray-600 mb-4">{section.content}</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 md:gap-8 my-8">
              {section.items?.map((topic, topicIndex) => (
                <article
                  ...
                  className="flex flex-col gap-1 p-4 rounded-md bg-white/60 backdrop-blur-md shadow-md hover:shadow-lg transition-transform hover:-translate-y-0.5"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h5 className="text-md sm:text-base font-semibold text-amber-700">{topic.title}</h5>
                      <p className="text-md sm:text-base text-gray-600 line-clamp-3 my-3">{topic.description}</p>
                    </div>
                  </div>
                  <div className="mt-auto flex items-center gap-3">
                    {topic.link ? (
                      ...
                      <span aria-disabled className="inline-flex items-center gap-2 text-sm font-medium text-amber-700 opacity-60 cursor-not-allowed">
                        Learn more about {topic.title}
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                        </svg>
              ...
              {section.points?.map((point, index) => (
                <div key={`${section.id}-point-${index}`} className="rounded-md p-5 bg-gradient-to-br from-amber-100 via-pink-50 to-indigo-50 shadow-md text-lg font-semibold text-gray-800 mb-3 border border-amber-300/40">
                  {point}
                </div>
              ))}
            </div>
