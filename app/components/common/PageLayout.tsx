            ...
            <div className="px-4 py-5 text-md sm:text-base leading-relaxed font-normal">
              <div className="text-center text-md sm:text-base leading-relaxed font-normal">
                <div className="inline-block relative text-md sm:text-base leading-relaxed font-normal">
                  {title && (<h1 className={`bg-clip-text bg-gradient-to-r ${h2Color} px-8 py-2 text-lg sm:text-xl md:text-2xl font-semibold drop-shadow-lg animate-gradient-x`}>
                    {title}
                  </h1>)}
      ...
      {isMounted && showSimilarCategories && (
        <>
          ...
          <button
            ...
            className="fixed right-0 top-1/2 z-40 -translate-y-1/2 rounded-l-2xl sd-explore-toggle px-1 py-3 bg-amber-500 transition-all duration-300 focus:outline-none cursor-pointer"
          >
            <span className="flex flex-col items-center gap-1 text-md leading-relaxed font-normal">
              <svg
                className={`h-5 w-5 text-white transition-transform duration-300 ${panelOpen ? 'rotate-180' : ''}`}
                fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"
              >
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" />
              </svg>
              <span className="uppercase text-white sd-vertical-lr text-md sm:text-base leading-relaxed font-normal">
                Explore
              </span>
            </span>
          </button>
          ...
          {panelOpen && (
            <div
              className="fixed inset-0 z-40 sd-explore-backdrop transition-opacity duration-300 text-md sm:text-base leading-relaxed font-normal"
          ...
          {/* Slide-in panel */}
          <aside className={`fixed right-0 top-0 z-50 flex h-full w-85 max-w-[85vw] flex-col sd-explore-panel transition-transform duration-300 ease-out ${panelOpen ? 'translate-x-0' : 'translate-x-full'
            }`}
            ...
            <div className="flex items-center justify-between border-[#d8a25a]/30 px-5 py-4 text-md sm:text-base leading-relaxed font-normal">
              <div className="flex items-center gap-2 text-md sm:text-base leading-relaxed font-normal">
                <span className="inline-flex h-7 w-7 items-center justify-center rounded-full sd-explore-badge text-md sm:text-base leading-relaxed font-normal">✦</span>
                <span className="uppercase text-amber-900 text-md sm:text-base leading-relaxed font-normal">Explore</span>
              </div>
              <button
                type="button"
                onClick={() => setPanelOpen(false)}
                aria-label="Close explore panel"
                className="inline-flex h-8 w-8 items-center justify-center rounded-full sd-explore-close-btn transition-all duration-200"
              >
