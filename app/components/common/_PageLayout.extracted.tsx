...
export default function PageLayout({
  ...
  const hasPolicyOrPrivacySegment = contentSegments.some(segment => segment.includes('policy') || segment.includes('privacy'));
  const showSimilarCategories = !excludedPrimarySegments.has(primarySegment) && !hasPolicyOrPrivacySegment;
  ...
  return <>
      ...
      {showSimilarCategories && <>
          ...
          <button type="button" onClick={togglePanel} aria-label={panelOpen ? 'Close explore panel' : 'Open explore panel'} className="fixed right-0 top-1/2 z-40 -translate-y-1/2 rounded-l-2xl border border-r-0 border-[#d8a25a]/60 bg-linear-to-b from-[#fff7ed] to-[#fde7c7] px-2 py-5 shadow-[−4px_4px_20px_rgba(166,61,23,0.14)] transition-all duration-300 hover:bg-linear-to-b hover:from-[#fde7c7] hover:to-[#f8d7a0] hover:shadow-[−6px_6px_24px_rgba(166,61,23,0.20)] focus:outline-none focus:ring-2 focus:ring-[#d97706]/50">
            <span className="flex flex-col items-center gap-1">
              <svg className={`h-5 w-5 text-[#7a2e1f] transition-transform duration-300 ${panelOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" />
              </svg>
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#9a3412] [writing-mode:vertical-lr]"> {page.explore} </span>
            </span>
          </button>

          {/* Backdrop overlay */}
          {panelOpen && <div className="fixed inset-0 z-40 bg-[#5b2d12]/20 backdrop-blur-[2px] transition-opacity duration-300" onClick={() => setPanelOpen(false)} aria-hidden="true" />}

          {/* Slide-in panel */}
          <aside className={`fixed right-0 top-0 z-50 flex h-full w-85 max-w-[85vw] flex-col border-l border-[#d8a25a]/40 bg-linear-to-b from-[#fffaf3] via-[#fdf0d7] to-[#fff8ef] shadow-[−12px_0_40px_rgba(166,61,23,0.12)] transition-transform duration-300 ease-out ${panelOpen ? 'translate-x-0' : 'translate-x-full'}`} aria-label="Explore related topics">
            {/* Panel header */}
            <div className="flex items-center justify-between border-b border-[#d8a25a]/30 bg-linear-to-r from-[#fff7ed] to-[#fde7c7] px-5 py-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-black uppercase tracking-[0.2em] text-[#7a2e1f]">{page.explore}</span>
              </div>
              <button type="button" onClick={() => setPanelOpen(false)} aria-label="Close explore panel" className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-[#d8a25a]/40 bg-[#fffaf2] text-[#7a2e1f] transition-all duration-200 hover:bg-[#fde7c7] hover:shadow-[0_2px_8px_rgba(146,64,14,0.12)]">
