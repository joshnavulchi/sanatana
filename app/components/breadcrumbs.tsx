...
export default function Breadcrumbs({ items, locale }: { items?: CrumbInput[]; locale?: string }) {
  ...
  return (
    <nav aria-label="Breadcrumb" className="inline-flex relative animate-fadeInUp">
      <div className="flex bg-gradient-to-r from-amber-50 via-pink-50 to-rose-100 items-center rounded-2xl px-3 py-2 shadow-lg border-2 border-amber-200/50 text-amber-800 text-sm leading-relaxed font-normal animate-gradient-x">
        <svg className="w-5 h-5 text-amber-800 flex-shrink-0 animate-pulse" fill="currentColor" viewBox="0 0 20 20">
        ...
        <ol className="flex items-center pl-0! mb-0!">
          {normalized.map((it, idx) => {
            ...
            return (
              <li key={idx} className="inline-flex items-center animate-fadeIn" aria-current={isLast ? 'page' : undefined}>
                {it.href && !isLast ? (
                  <Link
                    href={it.href}
                    className="text-sm font-semibold hover:text-pink-600 transition-colors duration-200 relative after:absolute after:bottom-0 after:left-0 after:w-0 after:h-0.5 after:bg-pink-600 hover:after:w-full after:transition-all after:duration-300 rounded-full hover:bg-pink-50"
                  >
                    {it.label}
                  </Link>
                ) : (
                  <span className="text-pink-700 bg-pink-100/60 rounded-full text-sm leading-relaxed font-semibold animate-pulse">{it.label}</span>
                )}
