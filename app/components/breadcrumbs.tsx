            ...
            return (
              <li key={idx} className="inline-flex items-center mb-0!" aria-current={isLast ? 'page' : undefined}>
                {it.href && !isLast ? (
                  <Link
                    href={it.href}
                    className="text-xs md:base-sm hover:text-amber-600 transition-colors duration-200 relative after:absolute after:bottom-0 after:left-0 after:w-0 after:h-0.5 after:bg-amber-600 hover:after:w-full after:transition-all after:duration-300"
                  >
                    {it.label}
                  </Link>
                ) : (
                  <span className="text-xs md:base-sm text-amber-700 bg-amber-100/50 px-4 py-1 my-1 rounded-md">{it.label}</span>
                )}
