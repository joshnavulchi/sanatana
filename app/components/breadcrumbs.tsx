...
export default function Breadcrumbs({ items, locale }: { items?: CrumbInput[]; locale?: string }) {
  ...
  return (
    <nav aria-label="Breadcrumb" className="inline-flex relative">
      <div className="flex bg-white items-center rounded-lg px-3 py-1 shadow-sm border-amber-200/50 text-amber-800 text-xs leading-relaxed font-normal">
            ...
            return (
              <li key={idx} className="inline-flex items-center" aria-current={isLast ? 'page' : undefined}>
                {it.href && !isLast ? (
                  <Link
                    href={it.href}
                    className="text-xs hover:text-amber-600 transition-colors duration-200 relative after:absolute after:bottom-0 after:left-0 after:w-0 after:h-0.5 after:bg-amber-600 hover:after:w-full after:transition-all after:duration-300"
                  >
                    {it.label}
                  </Link>
                ) : (
                  <span className="text-amber-700 bg-amber-100/50 px-4 py-1 my-1 rounded-md text-xs leading-relaxed font-normal">{it.label}</span>
                )}
