...
const page = {
  d_love: "We'd love to hear from you",
...
export default function ContactForm({
  ...
  const defaults: Field[] = useMemo(() => [{
    name: 'name',
    type: 'text',
    label: localeObj?.contactForm?.name || 'Name',
    placeholder: localeObj?.contactForm?.placeholderName || ''
  }, {
    name: 'email',
    type: 'email',
    label: localeObj?.contactForm?.email || 'Email',
    placeholder: localeObj?.contactForm?.placeholderEmail || '',
    required: true
  }, {
    name: 'message',
    type: 'textarea',
    label: localeObj?.contactForm?.message || 'Message',
    placeholder: localeObj?.contactForm?.placeholderMessage || '',
    required: true
  }], [localeObj]);
  ...
  const handleChange = (name: string, v: string) => setValues(s => ({
    ...s,
    [name]: v
  }));
    ...
    <div className="bg-gradient-to-br from-white via-amber-50/30 to-orange-50/30 rounded-3xl shadow-2xl border-2 border-amber-200/50 overflow-hidden">
      ...
      <div className="relative bg-gradient-to-r from-amber-500 via-orange-500 to-amber-500 p-4 text-center">
        ...
        <div className="relative z-10">
          ...
          <h3 className="text-2xl text-white mb-2">{page.get_touch}</h3>
          <p className="text-white/90 text-xl md:text-lg">{page.d_love}</p>
        </div>
      </div>
      ...
      <div className="p-4 space-y-4">
        {usedFields.map(f => <div key={f.name} className="group">
          ...
          {f.type === 'textarea' ? <textarea value={values[f.name] || ''} onChange={e => handleChange(f.name, e.target.value)} className="w-full px-4 py-3 bg-white border-2 border-amber-200 rounded-xl focus:border-amber-500 focus:ring-4 focus:ring-amber-500/20 outline-none transition-all duration-300 text-gray-900 placeholder-gray-400 resize-none shadow-sm hover:shadow-md" rows={6} placeholder={f.placeholder || ''} required={!!f.required} /> : <input type={f.type || 'text'} value={values[f.name] || ''} onChange={e => handleChange(f.name, e.target.value)} className="w-full px-4 py-3 bg-white border-2 border-amber-200 rounded-xl focus:border-amber-500 focus:ring-4 focus:ring-amber-500/20 outline-none transition-all duration-300 text-gray-900 placeholder-gray-400 shadow-sm hover:shadow-md" placeholder={f.placeholder || ''} required={!!f.required} />}
        </div>)}

        {/* Success/Error Messages */}
        {successMessage && <div className="flex items-center gap-3 p-4 bg-green-50 border-2 border-green-200 rounded-xl text-green-700 animate-fade-in">
          <svg className="w-5 h-5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
          </svg>
          <span className="font-medium">{successMessage}</span>
        </div>}
        {errorMessage && <div className="flex items-center gap-3 p-4 bg-red-50 border-2 border-red-200 rounded-xl text-red-700 animate-fade-in">
          <svg className="w-5 h-5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
