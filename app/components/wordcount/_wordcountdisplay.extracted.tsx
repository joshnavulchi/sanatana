...
export default function WordCountDisplay({
  ...
  useEffect(() => {
    const compute = () => {
      ...
      const text = root instanceof Document ? root.body?.innerText || '' : (root as HTMLElement)?.innerText ?? (root as HTMLElement)?.textContent ?? '';
  ...
  const wpm = typeof process.env.NEXT_PUBLIC_READING_WPM !== 'undefined' ? Math.max(50, Number(process.env.NEXT_PUBLIC_READING_WPM) || 200) : 200;
  const totalSeconds = Math.round(total / wpm * 60);
  const minutes = Math.floor(totalSeconds / 60);
  const seconds = totalSeconds % 60;
  const timeLabel = minutes > 0 ? `${minutes} min${minutes > 1 ? '' : ''}` : `${seconds} sec`;
  return <span className="wordcount text-gray-900" role="status" aria-live="polite">
      <span className="text-md label">{page.read_time}</span>
