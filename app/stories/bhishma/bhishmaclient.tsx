...
export default function BhishmaClient() {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!mounted) return;
      const title = String(ns?.title || 'Bhishma');
  ...
  if (isLoading && !data.title) {
    return (
      <PageLayout title="" breadcrumbs={[{ labelKey: 'Home', href: '/' }, { label: 'Bhishma' }]} className="layout-md">
            ...
            <div key={i} className="p-4 border rounded-md bg-white/60">
              {typeof item === 'object' ? Object.entries(item).map(([k, v]) => (
                <div key={k} className="mb-2">
                  ...
                  <div className="text-md md:text-md text-gray-700">{renderValue(v, k)}</div>
                </div>
              )) : <div>{String(item)}</div>}
            </div>
          ))}
      ...
      return (
        <div className="space-y-3">
          {Object.entries(value).map(([k, v]) => (
            <div key={k}>
              <div className="text-md font-semibold mb-1">{fmtLabel(k)}</div>
              <div className="text-md md:text-md text-gray-700">{renderValue(v, k)}</div>
            </div>
          ))}
        </div>
  ...
  return (
    <PageLayout
      ...
      className="layout-md"
    >
      <div id="bhishma-content" className="relative px-3 md:px-6 py-12 md:py-16 bg-gradient-to-br from-emerald-50 via-emerald-100 to-emerald-50 rounded-2xl border-l-4 border-emerald-500 shadow-lg overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          {data.intro && <p className="text-md leading-relaxed">{data.intro}</p>}
        </div>
      </div>
