            ...
            <div key={i} className="p-4 border rounded-md bg-white/60">
              {typeof item === 'object' ? Object.entries(item).map(([k, v]) => (
                <div key={k} className="mb-2">
                  ...
                  <div className="text-xl md:text-lg md:text-md text-gray-700">{renderValue(v, k)}</div>
                </div>
              )) : <div>{String(item)}</div>}
            </div>
          ))}
      ...
      return (
        <div className="space-y-3">
          {Object.entries(value).map(([k, v]) => (
            <div key={k}>
              <div className="text-lg md:text-md font-semibold mb-1">{fmtLabel(k)}</div>
              <div className="text-xl md:text-lg md:text-md text-gray-700">{renderValue(v, k)}</div>
            </div>
          ))}
        </div>
