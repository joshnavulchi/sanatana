...
export default function VishnuClient() {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!mounted) return;
      const title = String(ns?.title || 'Vishnu');
  ...
  if (isLoading && !data.title) {
    return (
      <PageLayout
        ...
        title=""
        breadcrumbs={[{ labelKey: 'Home', href: '/' }, { label: 'Vishnu' }]}
        className="layout-md"
      >
  ...
  return (
    <PageLayout
      ...
      className="layout-md"
    >
      <div id="vishnu-content" className="relative px-3 md:px-6 py-12 md:py-16 bg-gradient-to-br from-emerald-50 via-emerald-100 to-emerald-50 rounded-2xl border-l-4 border-emerald-500 shadow-lg overflow-hidden">
