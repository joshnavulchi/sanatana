...
export default function ShivaClient() {
  ...
  const renderValue = (value: any, key?: string) => {
    ...
    if (typeof value === 'string') return <div className="text-md md:text-md mb-3">{value}</div>;
    ...
    if (Array.isArray(value)) {
      ...
      return (
        <div className="space-y-3">
          {value.map((v, i) => (
            <div key={i} className="text-md md:text-md">{renderValue(v)}</div>
          ))}
        </div>
      );
    }
    ...
    return <div className="text-md md:text-md">{String(value)}</div>;
  };
  ...
  return (
    <PageLayout
      ...
      className="layout-md"
    >
      <div id="shiva-content" className="relative px-3 md:px-6 py-12 md:py-16 bg-gradient-to-br from-emerald-50 via-emerald-100 to-emerald-50 rounded-2xl border-l-4 border-emerald-500 shadow-lg overflow-hidden">
