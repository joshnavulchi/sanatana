...
export default function ShivaClient() {
  ...
  const renderValue = (value: any, key?: string) => {
    ...
    if (typeof value === 'string') return <div className="text-xl md:text-lg md:text-md mb-3">{value}</div>;
    ...
    if (Array.isArray(value)) {
      ...
      return (
        <div className="space-y-3">
          {value.map((v, i) => (
            <div key={i} className="text-xl md:text-lg md:text-md">{renderValue(v)}</div>
          ))}
        </div>
      );
    }
    ...
    return <div className="text-xl md:text-lg md:text-md">{String(value)}</div>;
  };
