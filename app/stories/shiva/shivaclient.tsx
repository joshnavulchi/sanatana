...
export default function ShivaClient() {
  ...
  const renderValue = (value: any, key?: string) => {
    ...
    if (typeof value === 'string') return <div className="text-md mb-3">{value}</div>;
    ...
    if (Array.isArray(value)) {
      ...
      return (
        <div className="space-y-3">
          {value.map((v, i) => (
            <div key={i} className="text-md">{renderValue(v)}</div>
          ))}
        </div>
      );
    }
    ...
    return <div className="text-md">{String(value)}</div>;
  };
