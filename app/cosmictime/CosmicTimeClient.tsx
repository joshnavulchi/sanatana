...
function RenderValue({ value }: { value: any }) {
  if (value === null || value === undefined) return <em className="text-gray-500 dark:text-gray-400 italic">—</em>;

  if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean') {
    return <p className="text-lg sm:text-base leading-relaxed text-gray-700 dark:text-gray-200">{String(value)}</p>;
  }
  ...
  if (Array.isArray(value)) {
      ...
      if (value.every((v) => typeof v !== 'object')) {
        return (
          <ul className="flex flex-wrap gap-3">
            {value.map((item, i) => (
              <li key={i} className="inline-flex items-center px-3 py-1 rounded-full bg-indigo-50 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-200 text-sm font-medium">
                {String(item)}
              </li>
            ))}
          </ul>
    ...
    return (
      <div className="space-y-4">
        {value.map((item, i) => (
          <div key={i} className="p-6 border border-transparent rounded-2xl bg-white/80 dark:bg-gray-900/60 shadow-lg hover:shadow-2xl transform hover:-translate-y-1 transition-all duration-300">
            <h4 className="text-lg font-semibold mb-2 text-indigo-700 dark:text-indigo-300">Item {i + 1}</h4>
      ...
      <dl className="grid grid-cols-1 gap-y-4">
        {Object.entries(value).map(([k, v]) => (
          <div key={k} className="flex gap-4 items-start">
            <dt className="font-semibold w-40 text-indigo-700 dark:text-indigo-300 uppercase text-sm">{k.replace(/_/g, ' ')}</dt>
            <dd className="flex-1">
              <RenderValue value={v} />
            </dd>
          </div>
  ...
  if (isLoading && !pageTitle) {
    return (
      <PageLayout
        ...
        className="layout-md"
      >
        <div className="flex items-center justify-center py-24 bg-gradient-to-b from-indigo-50 to-white dark:from-gray-900/30 rounded-2xl">
          <Loader />
        </div>
      </PageLayout>
    );
      ...
      >
        ...
        <div id="cosmictime-content" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 space-y-4">
          ...
          {Object.entries(cosmicTime).map(([key, value]) => (
            <section
              key={key}
              className="relative overflow-hidden rounded-3xl bg-gradient-to-tr from-white/80 to-indigo-50 dark:from-gray-900/60 dark:to-gray-800/30 p-6 shadow-xl hover:shadow-2xl transform hover:-translate-y-1 transition-all duration-500"
            >
              ...
              <div className="prose max-w-none text-lg sm:text-base leading-relaxed text-gray-700 dark:text-gray-200">
          ...
          {ns?.faq && (
            <div>
              <h3 className="text-2xl sm:text-3xl font-semibold mb-4 text-gray-900 dark:text-white">{ns.faq.heading || 'FAQ'}</h3>
