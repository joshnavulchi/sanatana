...
export default function DailypujaClient() {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!mounted) return;
      const container = ns ?? {};
      const title = String(container?.title || 'Daily Puja');
  ...
  if (isLoading && !data.title) {
    return (
      <PageLayout metaKey="dailypuja_practices" title="" breadcrumbs={[{ labelKey: 'Home', href: '/' }, { label: 'Daily Puja' }]} className="layout-md">
        ...
        <div className="w-full lg:w-3/4">
          <div className="relative px-3 md:px-6 py-12 md:py-16 bg-gradient-to-br from-yellow-50 via-amber-50 to-yellow-50 rounded-2xl overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-lg md:text-md md:text-xl md:text-lg md:text-md leading-relaxed mb-6">{data.intro}</p>
