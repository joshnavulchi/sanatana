...
function SectionCard({ item, index }: { item: Record<string, unknown>; index: number }) {
  ...
  const shells = [
    'bg-gradient-to-br from-[#e0e7ff] via-[#f3e8ff] to-[#f0fdfa]',
    'bg-gradient-to-br from-[#fef9c3] via-[#fcd34d] to-[#fef3c7]',
    'bg-gradient-to-br from-[#f0fdfa] via-[#a7f3d0] to-[#f3e8ff]',
  ];

  return (
    <div className={`relative overflow-hidden rounded-3xl border-2 border-[#a78bfa]/30 p-4 md:p-8 ${shells[index % 3]} shadow-xl transition-all duration-500 hover:shadow-2xl hover:scale-[1.025] group motion-safe:animate-fadeIn`}>
      ...
      <div className="flex items-center gap-3 mb-5 pl-2">
        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl text-sm font-semibold text-[#fffaf0] shadow-[0_4px_20px_rgba(122,46,31,0.25)] bg-gradient-to-br from-[#a78bfa] to-[#fcd34d] animate-bounceIn">
          {index + 1}
        </span>
        <h3 className="text-xl md:text-2xl font-semibold text-transparent bg-clip-text bg-gradient-to-r from-[#3b3270] via-[#8b6914] to-[#e0a632] drop-shadow-lg animate-gradient-x">{section}</h3>
      </div>
