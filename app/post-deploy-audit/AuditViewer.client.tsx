  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        <button className="text-lg sm:text-base text-amber-800" onClick={() => setShowRaw((s) => !s)}>{showRaw ? 'Hide JSON' : 'Show raw JSON'}</button>
      </div>

      <div className="mb-4 bg-amber-50 p-4 rounded-lg shadow-sm">
        <div className="text-lg sm:text-base">Generated: <strong>{report.generatedAt ?? 'N/A'}</strong></div>
        <div className="text-lg sm:text-base">Pages: <strong>{report.totalPages ?? pages.length}</strong></div>
        <div className="text-lg sm:text-base">Passed: <strong className="text-green-700">{report.passed ?? '—'}</strong> Failed: <strong className="text-red-700">{report.failed ?? '—'}</strong></div>
      </div>
        ...
        {paginated.map((p, i) => (
          <div key={`${p.page}-${i}`} className="p-4 rounded-lg border shadow-sm flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="text-lg sm:text-base text-gray-700">Status: <span className="font-medium">{p.status}</span> — Score: {p.score ?? '—'}</div>
              {p.issues && p.issues.length > 0 && (
                <ul className="mt-2 list-disc pl-5 text-lg sm:text-base text-red-700">
