// @ts-nocheck
// Auto-generated index for folder 'zh-CN\vedas\atharvaveda'
// Imports local JSON files and child folder indexes, deep-merging them into one export.

import _j0 from './atharvaveda.json';
import _j1 from './structure.json';
import _d0 from './book1/index';
import _d1 from './book10/index';
import _d2 from './book2/index';
import _d3 from './book3/index';
import _d4 from './book4/index';
import _d5 from './book5/index';
import _d6 from './book6/index';
import _d7 from './book7/index';
import _d8 from './book8/index';
import _d9 from './book9/index';



function deepMerge(target: any, source: any) {
  if (source === undefined) return target;
  if (Array.isArray(target) && Array.isArray(source)) {
    const out = target.slice();
    for (let i = 0; i < source.length; i++) out[i] = deepMerge(out[i], source[i]);
    return out;
  }
  if (target && typeof target === 'object' && source && typeof source === 'object') {
    const out: any = { ...target };
    for (const k of Object.keys(source)) out[k] = deepMerge(target ? target[k] : undefined, source[k]);
    return out;
  }
  return source;
}



const base: any = {};
const merged = ([_j0, _j1, _d0, _d1, _d2, _d3, _d4, _d5, _d6, _d7, _d8, _d9] as any[]).reduce((acc: any, cur: any) => deepMerge(acc, cur || {}), base as any);
export default merged;