...
def resolve_locales_root(locales_dir_arg: str | None) -> str:
    ...
    public_locales = os.path.join(repo_root, "public", "locales")
    public_data_locales = os.path.join(repo_root, "public", "data", "locales")

    if not locales_dir_arg:
        if os.path.isdir(default_locales):
            return default_locales
        if os.path.isdir(public_locales):
            return public_locales
        if os.path.isdir(public_data_locales):
            return public_data_locales
        return default_locales
...
def collect_locale_files(source_dir):
    """Recursively collect JSON locale files under the source directory."""
    files = []
...
def translate_text(text, target_lang, chunk_size=500):
    ...
    for i in range(0, len(text), chunk_size):
        ...
        try:
            ...
            print(f"WARNING: Translation not found for chunk (len={len(chunk)}). Keeping original chunk.")
            translated_chunks.append(chunk)
        except Exception as e:
            # Generic fallback: keep original text to avoid failing entire run
            print(f"WARNING: Translator error: {e}. Keeping original chunk.")
            translated_chunks.append(chunk)
    ...
    try:
        ...
        if os.path.exists(path):
            ...
            try:
                ...
                print(f"WARNING: failed to create backup for {path}")

        # Atomic replace
        os.replace(tmp_path, path)

        # Remove the temporary backup once translation is complete
        if bak_path and os.path.exists(bak_path):
            try:
                os.remove(bak_path)
            except Exception:
                print(f"WARNING: failed to remove backup file {bak_path}")
        ...
        try:
            ...
            pass
        print(f"WARNING: Failed to write JSON to {path}: {e}")
        return False
...
def main():
    ...
    parser.add_argument("--files", help="Comma-separated filenames from the locale source directory to translate (e.g. donate.json,home.json)")
    parser.add_argument("--locales", help="Comma-separated locale codes to translate into (e.g. es,fr)")
    parser.add_argument(
        "--locales-dir",
        help="Path to locales directory (default: <repo>/locales, public/locales, or public/data/locales)",
    )
    ...
    if not os.path.isdir(source_dir):
        ...
        print("Tip: run from the repo root or pass --locales-dir=./locales or --locales-dir=./public/locales")
        sys.exit(1)

    non_interactive = args.non_interactive

    files = collect_locale_files(source_dir)
    if not files:
    ...
    for filename in filenames:
        input_path = os.path.join(source_dir, *filename.split("/"))
        ...
        try:
            ...
            if args.locale_workers is not None:
                locale_workers = max(1, args.locale_workers)
            elif locale_workers_env:
                locale_workers = max(1, int(locale_workers_env))
            else:
                locale_workers = min(32, max(1, os.cpu_count() or 1))
        except Exception:
            locale_workers = min(32, max(1, os.cpu_count() or 1))
        ...
        def _translate_one_locale(target_lang):
            ...
            output_path = os.path.join(target_dir, *filename.split("/"))
            ...
            if total_translated == 0:
                print(f"\nNOTE: No untranslated strings found for {filename} in {target_lang}. Skipping.")
                return (target_lang, 0)

            ok = safe_write_json(output_path, translated_data)
            if ok:
                print(f"\nDONE: Finished translating {filename} into {target_lang}")
            else:
                print(f"\nERROR: Failed to save translations for {filename} into {target_lang}; original file preserved.")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=locale_workers) as exec:
            ...
            for fut in concurrent.futures.as_completed(futures):
                ...
                try:
                    ...
                    print(f"\nWARNING: Error translating {filename} for {lc}: {e}")
